...
if (function($) {
        ...
        function filterSelection(selection, obj) {
            function finishImageProcessing(newAttachments) {
                ...
                var template = vc.template($("#vc_css-editor-image-block").html(), _.defaults({}, {
                    variable: "img"
                }, vc.templateOptions.custom));
                obj._css_editor.$el.find(".vc_background-image .vc_image").html(template(objects[0])), $(".media-modal").removeClass("processing-media")
            }
            var ids;
        ...
        media.controller.VcCssSingleImage = media.controller.VcSingleImage.extend({
            ...
            updateSelection: function() {
                ...
                "undefined" != typeof ids && "" !== ids && -1 !== ids && (attachments = _.map(ids.toString().split(/,/), function(id) {
        ...
        VcCssEditor = vc.CssEditor = Backbone.View.extend({
            attrs: {},
            layouts: ["margin", "border-width", "paddiing"],
            ...
            addBackgroundImage: function(e) {
                return e.preventDefault(), window.vc_selectedFilters = {}, this.image_media ? this.image_media.open("vc_editor") : (this.image_media = media({
                ...
                }, this.image_media), this.image_media.state("vc_single-image").on("select", this.setBgImage), void this.image_media.open("vc_editor"))
            },
            setBgImage: function() {
            ...
            setCurrentBgImage: function(value) {
                var image_split, template, image_regexp = /([^\?]+)(\?id=\d+){0,1}/,
                    url = "",
                    id = "";
                ...
                })) : value.match(image_regexp) && (image_split = value.split(image_regexp), url = image_split[1], image_split[2] && (id = image_split[2].replace(/[^\d]+/, "")), template = vc.template($("#vc_css-editor-image-block").html(), _.defaults({}, {
                    variable: "img"
                }, vc.templateOptions.custom)), this.$el.find(".vc_background-image .vc_image").html(template({
                    url: url,
                    id: id
            ...
            removeImage: function(e) {
                ...
                e.preventDefault(), $control.parent().remove()
            },
