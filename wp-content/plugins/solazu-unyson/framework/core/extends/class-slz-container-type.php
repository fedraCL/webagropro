 ...
 */
abstract class SLZ_Container_Type
{
	/**
	...
	private function prepare($id, &$option, &$data)
	{
		$data = array_merge(
			array(
				'id_prefix'   => slz()->backend->get_options_id_attr_prefix(),   // attribute id prefix
				'name_prefix' => slz()->backend->get_options_name_attr_prefix(), // attribute name prefix
			),
			$data
		);
		...
		if (!isset($option['title'])) {
			$option['title'] = slz_id_to_title($id);
		}

		$option['attr']['class'] = 'slz-container slz-container-type-'. $option['type'] .(
	...
	final public static function register($container_type_class) {
		...
		if ($registration_access_key === null) {
			$registration_access_key = new SLZ_Access_Key('slz_container_type');
		}

		slz()->backend->_register_container_type($registration_access_key, $container_type_class);
	}
}