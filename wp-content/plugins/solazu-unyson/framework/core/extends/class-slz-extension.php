...
abstract class SLZ_Extension
{
	...
	/** @var SLZ_Extension_Manifest */
	public $manifest;

	/** @var string Key used in SLZ_Cache to store data about extensions */
	private static $cache_key = 'slz_ext';
	...
	{
		if (!self::$access_key) {
			self::$access_key = new SLZ_Access_Key('extension');
		}
		...
		{
			$variables = slz_get_variables_from_file($this->locate_path('/manifest.php'), array('manifest' => array()));
			$manifest  = $variables['manifest'];
			unset($variables);

			if (empty($manifest['name'])) {
				$manifest['name'] = slz_id_to_title($this->get_name());
			}

			$this->manifest = new SLZ_Extension_Manifest($manifest);
		}
	}
	 ...
	 *
	 * Usage:
	 * SLZ_Cache::get( $this->get_cache_key('/some/key') )
	 *
	...
	final protected function render_view($name, $view_variables = array(), $return = true)
	{
		...
		return slz_render_view($full_path, $view_variables, $return);
	}
	...
	{
		if ($access_key->get_key() !== 'slz_extensions') {
	...
	{
		return slz()->extensions->_get_extension_tree(self::$access_key, $this->get_name());
	}
	...
	{
		try {
			return SLZ_File_Cache::get($cache_key = 'ext:'. $this->get_name() .':path:'. $rel_path);
	...
	{
		try {
			return SLZ_File_Cache::get($cache_key = 'ext:'. $this->get_name() .':uri:'. $rel_path);
		} catch (SLZ_File_Cache_Not_Found_Exception $e) {
			$result = false;
			$locations = $this->customizations_locations;
			$locations[$this->get_path()] = $this->get_uri();
	...
	/**
	 * @return SLZ_Extension|null if has no parent extension
	 */
	...
	final public function get_child($child_extension_name)
	{
		...
		if (isset($active_tree[$child_extension_name])) {
			return slz()->extensions->get($child_extension_name);
		} else {
			return null;
		}
	}
	...
	final public function get_children()
	{
		...
		foreach ($active_tree as $extension_name => &$sub_extensions) {
			$result[$extension_name] = slz()->extensions->get($extension_name);
		}
	...
	final public function get_config($key = null)
	{
		...
		try {
			...
			foreach (array_reverse($locations) as $path => $uri) {
				...
				if (file_exists($config_path)) {
					$variables = slz_get_variables_from_file($config_path, array('cfg' => null));
		...
		return $key === null ? $config : slz_akg($key, $config);
	}
	...
	{
		try {
			return SLZ_Cache::get($cache_key = $this->get_cache_key('/options/'. $name));
		} catch (SLZ_Cache_Not_Found_Exception $e) {
			...
			SLZ_Cache::set($cache_key, $variables['options']);
	...
	{
		try {
			return SLZ_Cache::get($cache_key = $this->get_cache_key('/settings_options'));
		} catch (SLZ_Cache_Not_Found_Exception $e) {
			if (file_exists($path = $this->get_path('/settings-options.php'))) {
				$variables = slz_get_variables_from_file($path, array('options' => array()));
			} else {
				$variables = array('options' => array());
			}
	...
	final public function get_db_settings_option( $option_id = null, $default_value = null, $get_original_value = null ) {
		return slz_get_db_ext_settings_option( $this->get_name(), $option_id, $default_value, $get_original_value );
	}
	...
	final public function set_db_settings_option( $option_id = null, $value ) {
		slz_set_db_ext_settings_option( $this->get_name(), $option_id, $value );
	}
	...
	final public function get_db_data( $multi_key = null, $default_value = null, $get_original_value = null ) {
		return slz_get_db_extension_data( $this->get_name(), $multi_key, $default_value, $get_original_value );
	}
	...
	final public function set_db_data( $multi_key = null, $value ) {
		slz_set_db_extension_data( $this->get_name(), $multi_key, $value );
	}
	...
	final public function get_user_data( $user_id, $keys = null ) {
		return slz_get_db_extension_user_data($user_id, $this->get_name(), $keys);
	}
	...
	final public function set_user_data( $user_id, $value, $keys = null ) {
		return slz_set_db_extension_user_data($user_id, $this->get_name(), $value, $keys);
	}
	 ...
	 *
	 * @param SLZ_Extension $child_extension_instance
	 * @return bool
	 * @internal
	 */
	public function _child_extension_is_valid($child_extension_instance)
	{
		return is_subclass_of($child_extension_instance, 'SLZ_Extension');
	}
