 ...
 * @internal
 */
final class _SLZ_Extensions_Manager
{
	/**
	 * @var SLZ_Form
	 */
	...
	private static function get_access_key() {
		if (!self::$access_key) {
			self::$access_key = new SLZ_Access_Key('slz_ext_manager');
		}
	...
	public function __construct()
	{
		...
		{
			add_action('slz_plugin_pre_update', array($this, '_action_plugin_pre_update'));
			add_action('slz_plugin_post_update', array($this, '_action_plugin_post_update'));
		}
		...
		add_action('slz_after_plugin_activate', array($this, '_action_after_plugin_activate'), 100);
		...
		/** Actions */
		{
			add_action('slz_init', array($this, '_action_slz_init'));
			...
			if ($this->can_install()) {
				add_action('wp_ajax_slz_extensions_check_direct_fs_access', array($this, '_action_ajax_check_direct_fs_access'));
				add_action('wp_ajax_slz_extensions_install', array($this, '_action_ajax_install'));
				add_action('wp_ajax_slz_extensions_uninstall', array($this, '_action_ajax_uninstall'));
			}
		}

		/** Filters */
		{
			add_filter('slz_plugin_action_list', array($this, '_filter_plugin_action_list'));
		}
	}
	...
	{
		return 'slz-extensions';
	}

	private function get_cache_key($sub_key)
	{
		return 'slz_extensions_manager/'. $sub_key;
	}

	private function get_uri($append = '')
	{
		return slz_get_framework_directory_uri('/core/components/extensions/manager'. $append);
	}
	...
	private function get_nonce($form) {
		switch ($form) {
			case 'install':
				return array(
					'name' => '_nonce_slz_extensions_install',
					'action' => 'install',
				);
			case 'delete':
				return array(
					'name' => '_nonce_slz_extensions_delete',
					'action' => 'delete',
				);
			case 'activate':
				return array(
					'name' => '_nonce_slz_extensions_activate',
					'action' => 'activate',
				);
			case 'deactivate':
				return array(
					'name' => '_nonce_slz_extensions_deactivate',
					'action' => 'deactivate',
				);
			default:
				return array(
					'name' => '_nonce_slz_extensions',
					'action' => 'default',
				);
		}
	}
	...
	private function get_available_extensions()
	{
		try {
			...
			$extensions = slz_get_variables_from_file(
			...
			if (file_exists(
				$theme_available_ext_file = slz_fix_path(get_template_directory())
					. slz_get_framework_customizations_dir_rel_path( '/theme/available-extensions.php' )
			)) {
				require_once dirname( __FILE__ ) . '/includes/available-ext/class--slz-available-extensions-register.php';
				require_once dirname( __FILE__ ) . '/includes/available-ext/class-slz-available-extension.php';

				$register = new _SLZ_Available_Extensions_Register(self::get_access_key()->get_key());
				 ...
				 */
				slz_get_variables_from_file($theme_available_ext_file, array(), array('register' => $register));

				foreach ($register->_get_types(self::$access_key) as $extension) {
					/** @var SLZ_Available_Extension $extension */
					if (isset($extensions[ $extension->get_name() ])) {
						if ( !$extension->is_valid() ) {
							unset ( $extensions[ $extension->get_name() ] );
						}
						continue;
...

			{
				...
				$supported_extensions = slz()->theme->manifest->get('supported_extensions', array());
			...
			SLZ_Cache::set($cache_key, $extensions);
	...
	public function _action_ajax_check_direct_fs_access()
	{
		...
		if (SLZ_WP_Filesystem::has_direct_access(slz_get_framework_directory('/extensions'))) {
	...
	public function _action_ajax_install()
	{
		...
		$extension = (string)SLZ_Request::POST('extension');
	...
	public function _action_after_plugin_activate()
	{
		...
		$this->activate_extensions(
			array_fill_keys(
				array_keys(slz()->theme->manifest->get('supported_extensions', array())),
				array()
			)
		);
	...
	public function _action_plugin_pre_update()
	{
		...
		$tmp_dir = SLZ_WP_Filesystem::real_path_to_filesystem_path(
			slz_fix_path(WP_CONTENT_DIR) .'/tmp/slz-plugin-update-extensions-backup'
		);
		$extensions_dir = SLZ_WP_Filesystem::real_path_to_filesystem_path(
			slz_get_framework_directory('/extensions')
		);
		...
		do {
			if ($wp_filesystem->exists($tmp_dir)) {
				if (!$wp_filesystem->delete($tmp_dir, true, 'd')) {
					$error = __('Cannot remove the old extensions backup dir', 'slz');
					break;
				}
			}

			if (!SLZ_WP_Filesystem::mkdir_recursive($tmp_dir)) {
				$error = __('Cannot create the extensions backup dir', 'slz');
				break;
			}

			if (true !== copy_dir($extensions_dir, $tmp_dir)) {
				$error = __('Cannot backup the extensions', 'slz');
				break;
			}
		} while(false);
		...
		do {
			if ($wp_filesystem->exists($extensions_dir)) {
				...
				if (!$wp_filesystem->delete( $extensions_dir, true, 'd' )) {
					$error = __( 'Cannot clear the extensions directory', 'slz' );
					break;
				}

				if ( ! SLZ_WP_Filesystem::mkdir_recursive( $extensions_dir ) ) {
					$error = __( 'Cannot recreate the extensions directory', 'slz' );
					break;
				}
			}

			if (true !== copy_dir($tmp_dir, $extensions_dir)) {
				$error = __('Cannot recover the extensions', 'slz');
				break;
			}
		} while(false);
	...
	private function get_installed_extensions($reset_cache = false)
	{
		...
		try {
			...
			foreach (slz()->extensions->get_locations() as $location) {
	...
	private function read_extensions($location, &$list, $parent_extension_name = null)
	{
		...
		foreach ($paths as $extension_path) {
			...
			$valid = false;
			$ext = slz_ext($extension_name);

			if ( $ext != null && file_exists( $ext->locate_path( '/manifest.php' ) ) )
				$valid = $ext->locate_path( '/manifest.php' );
			else if ( file_exists($extension_path .'/manifest.php') )
				$valid = $extension_path .'/manifest.php';

			if (isset($list[$extension_name])) {
				// extension already found
			} elseif ( $valid != false ) {
				$vars = slz_get_variables_from_file( $valid, array(
					'manifest' => array(),
				));
				...
				$list[$extension_name] = array(
					...
					'children' => array(),
					'active'   => (bool)slz()->extensions->get($extension_name),
	...
	{
		return apply_filters('slz_tmp_dir', slz_fix_path(WP_CONTENT_DIR) .'/tmp') . $append;
	}

	/**
	 * @internal
	 */
	public function _action_slz_init()
	{
		$this->extension_settings_form = new SLZ_Form('slz_extension_settings', array(
		...
		if (is_admin() && $this->can_activate()) {
			$db_wp_option_name = 'slz_extensions_activation';
			...
			if ($db_wp_option_value = get_option($db_wp_option_name, array())) {
				...
				/**
				 * Fire the 'slz_extensions_after_activation' action
				 */
				if ($db_wp_option_value['activated']) {
					...
					if (!empty($succeeded_extensions)) {
						do_action('slz_extensions_after_activation', $succeeded_extensions);
					}
					if (!empty($failed_extensions)) {
						do_action('slz_extensions_activation_failed', $failed_extensions);
					}
				}

				/**
				 * Fire the 'slz_extensions_after_deactivation' action
				 */
				if ($db_wp_option_value['deactivated']) {
					...
					if (!empty($succeeded_extensions)) {
						do_action('slz_extensions_after_deactivation', $succeeded_extensions);
					}
					if (!empty($failed_extensions)) {
						do_action('slz_extensions_deactivation_failed', $failed_extensions);
					}
				}
	...
	private function activate_hidden_standalone_extensions()
	{
		...
		foreach (
			...
			array_diff_key($this->get_installed_extensions(), slz()->extensions->get_all())
			as $ext_name => $ext_data
		) {
			if ($ext_data['parent'] && !slz_ext($ext_data['parent'])) {
			...
			if (is_wp_error($collected)) {
				if (defined('WP_DEBUG') && WP_DEBUG) {
					if ($this->is_extensions_page()) {
						...
						SLZ_Flash_Messages::add('slz_ext_auto_activate_hidden_standalone',
							sprintf(__('Cannot activate hidden standalone extension %s', 'slz'),
								slz_akg('name', $ext_data['manifest'], slz_id_to_title($ext_name))
							),
							'error'
						);
					}
		...
		$option_name = slz()->extensions->_get_active_extensions_db_option_name();
	...
	public function _action_admin_menu()
	{
		...
		$icon = get_template_directory_uri().'/static/img/theme-option/'.slz()->theme->manifest->get('id').'_menu_icon.png';

		add_menu_page(
			slz()->theme->manifest->get('name'),
			slz()->theme->manifest->get('name'),
			$capability,
			slz()->theme->manifest->get('id'),
			array($this, '_display_welcome_page'),
			$icon,
			3
		);

		$data_log = array(
			'title'             => __( 'Changes Log', 'slz' ),
			'capability'        => $capability,
			'slug'              => slz()->theme->manifest->get('log_page_id'),
			'content_callback'  => array($this, '_display_log_page'),
		);
		add_submenu_page(
				slz()->theme->manifest->get('id'),
				$data_log['title'],
        ...
        $submenu[slz()->theme->manifest->get('id')][0][0] = apply_filters('slz_first_menu_title', esc_html__('Welcome', 'slz'));
	 ...
	 * @internal
	 */
	public function _display_welcome_page()
	{
		wp_enqueue_style('slz');
		if (
			file_exists(
				$theme_available_welcome_file = slz_fix_path(get_template_directory())
					. slz_get_framework_customizations_dir_rel_path( '/theme/views/welcome.php' )
			)
		) {

			slz_render_view( slz_fix_path(get_template_directory())
					. slz_get_framework_customizations_dir_rel_path( '/theme/views/welcome.php' ), array(), false
			);
			...
			SLZ_Flash_Messages::add(
				'slz-welcome-page',
				__('Cannot find welcome page. You can create welcome page at : /theme/framework-customizations/views/welcome.php.', 'slz'),
				'error'
			);
			$this->js_redirect();
			return;
	 ...
	 * @internal
	 */
	public function _display_log_page()
	{
		wp_enqueue_style('slz');
		if (
				file_exists(
						$theme_available_welcome_file = slz_fix_path(get_template_directory())
						. slz_get_framework_customizations_dir_rel_path( '/theme/views/change-log.php' )
				)
		) {
	
			slz_render_view( slz_fix_path(get_template_directory())
					. slz_get_framework_customizations_dir_rel_path( '/theme/views/change-log.php' ), array(), false
			);
			...
			SLZ_Flash_Messages::add(
					'slz-change-log-page',
					__('Cannot find change log page. You can create change log page at : /theme/framework-customizations/views/change-log.php.', 'slz'),
					'error'
			);
			$this->js_redirect();
			return;
	...
	private function display_list_page()
	{
		...
		/** Prepare extensions list for view */
		{
			...
			$disabled_extensions = apply_filters('slz_ext_manager_disable_extension', array());
				...
				$lists['available'][$ext_name] = array(
					...
					'thumbnail' => isset($ext_data['thumbnail'])
						...
						: (isset($lists['installed'][$ext_name])
							? slz_akg('thumbnail', $lists['installed'][$ext_name]['manifest'], $this->default_thumbnail)
			...
			foreach (slz()->theme->manifest->get('supported_extensions', array()) as $required_ext_name => $required_ext_data) {
				if (isset($lists['installed'][ $required_ext_name ])) {
					$lists['supported'][ $required_ext_name ] = array(
						'name'        => slz_akg( 'name', $lists['installed'][ $required_ext_name ]['manifest'], slz_id_to_title( $required_ext_name ) ),
						'description' => slz_akg( 'description', $lists['installed'][ $required_ext_name ]['manifest'], '' ),
					);
					...
					$lists['supported'][ $required_ext_name ] = array(
						'name'        => slz_id_to_title( $required_ext_name ),
						'description' => '',
					);
				}
			}
			...
			foreach ( $lists as $list_key => $list_data ) {
				...
				foreach ( $lists[ $list_key ] as $ext_name => $ext_data ) {

					if ( in_array( $ext_name, $disabled_extensions ) )

						unset ( $lists[ $list_key ][ $ext_name ] );
		...
		echo '<div id="slz-extensions-list-wrapper">';

		slz_render_view(dirname(__FILE__) .'/views/extensions-page.php', array(
			'lists' => &$lists,
			'link' => $this->get_link(),
	...
	{
		$flash_id = 'slz_extensions_install';

		if (!$this->can_install()) {
			SLZ_Flash_Messages::add(
				$flash_id,
				__('You are not allowed to install extensions.', 'slz'),
				'error'
			);
			$this->js_redirect();
			return;
		...
		if (array_key_exists('supported', $_GET)) {
			...
			if (empty($extensions)) {
				SLZ_Flash_Messages::add(
					$flash_id,
					__('All supported extensions are already installed.', 'slz'),
					'info'
				);
				$this->js_redirect();
				return;
			...
			$extensions = array_fill_keys(
				array_map( 'trim', explode( ',', SLZ_Request::GET( 'extension', '' ) )),
				array()
			);
		...
		{
			if (!class_exists('_SLZ_Extensions_Install_Upgrader_Skin')) {
				slz_include_file_isolated(
					dirname(__FILE__) .'/includes/class--slz-extensions-install-upgrader-skin.php'
				);
			}

			$skin = new _SLZ_Extensions_Install_Upgrader_Skin(array(
				'title' => $supported
					? _n('Install Compatible Extension', 'Install Compatible Extensions', count($extensions), 'slz')
					: _n('Install Extension', 'Install Extensions', count($extensions), 'slz'),
			));
		}
		...
		do {
			...
			if ($_SERVER['REQUEST_METHOD'] === 'POST') {
				if (!isset($_POST[$nonce['name']]) || !wp_verify_nonce($_POST[$nonce['name']], $nonce['action'])) {
					$skin->error(__('Invalid nonce.', 'slz'));
					break;
				}

				if (!SLZ_WP_Filesystem::request_access(
					slz_get_framework_directory('/extensions'), slz_current_url(), array($nonce['name'])
				)) {
					break;
				}
				...
				$wp_fs_tmp_dir = SLZ_WP_Filesystem::real_path_to_filesystem_path($this->get_tmp_dir());

				if ($wp_filesystem->exists($wp_fs_tmp_dir)) {
					if ( ! $wp_filesystem->rmdir( $wp_fs_tmp_dir, true ) ) {
						$skin->error(
							sprintf( __( 'Cannot remove temporary directory: %s', 'slz' ), $wp_fs_tmp_dir )
						);
					}
				}
				...
				slz_render_view(dirname(__FILE__) .'/views/install-form.php', array(
	...
	public function install_extensions(array $extensions, $opts = array())
	{
		...
		if (!$this->can_install()) {
			return new WP_Error(
				'access_denied',
				__('You have no permissions to install extensions', 'slz')
			);
		}
		...
		if (!$wp_filesystem || (is_wp_error($wp_filesystem->errors) && $wp_filesystem->errors->get_error_code())) {
			return new WP_Error(
				'fs_not_initialized',
				__('WP Filesystem is not initialized', 'slz')
			);
		}
		...
		while (!empty($extensions)) {
			...
			if (isset($installed_extensions[$extension_name])) {
				$result[$extension_name] = new WP_Error(
					'extension_installed',
					sprintf(__('Extension "%s" is already installed.', 'slz'), $this->get_extension_title($extension_name))
				);
				$has_errors = true;
			...
			if (!isset($available_extensions[ $extension_name ])) {
				$result[$extension_name] = new WP_Error(
					...
					sprintf(
						__('Extension "%s" is not available for install.', 'slz'),
				...
				while (!empty($available_extensions[$current_parent]['parent'])) {
					...
					if (!isset($available_extensions[$current_parent])) {
						$result[$extension_name] = new WP_Error(
							...
							sprintf(
								__('Parent extension "%s" not available.', 'slz'),
			 ...
			 */
			{
				$destination_path = array(
					'framework' => slz_get_framework_directory(),
					'theme' => slz_fix_path(get_template_directory()) . slz_get_framework_customizations_dir_rel_path()
				);
				...
				foreach ($parents as $parent_extension_name) {
					...
					if ($verbose) {
						$verbose_message = sprintf(__('Downloading the "%s" extension...', 'slz'),
						...
						if (is_subclass_of($verbose, 'WP_Upgrader_Skin')) {
							...
							echo slz_html_tag('p', array(), $verbose_message);
						}
					}
					...
					$wp_slz_downloaded_dir = $this->download(
						$parent_extension_name,
						$available_extensions[$parent_extension_name]
					);

					if (is_wp_error($wp_slz_downloaded_dir)) {
						if ($verbose) {
							$verbose_message = $wp_slz_downloaded_dir->get_error_message();
						...
						$result[$extension_name] = $wp_slz_downloaded_dir;
						$has_errors = true;
					...
					if ($verbose) {
						$verbose_message = sprintf(__('Installing the "%s" extension...', 'slz'),
							$this->get_extension_title($parent_extension_name)
						);
					...
					$merge_result = $this->merge_extension(
						$wp_slz_downloaded_dir,
						SLZ_WP_Filesystem::real_path_to_filesystem_path(
					...
					if ($verbose) {
						$verbose_message = sprintf(__('The %s extension has been successfully installed.', 'slz'),
							$this->get_extension_title($parent_extension_name)
						);
				...
				if ($verbose) {
					if (is_wp_error($activation_result)) {
						if (is_subclass_of($verbose, 'WP_Upgrader_Skin')) {
							...
							echo slz_html_tag('p', array(), $activation_result->get_error_message());
						}
		...
		do_action('slz_extensions_install', $result);
		...
		if (
			...
			if (
				...
				return new WP_Error(
					'installation_failed',
					_n('Cannot install extension', 'Cannot install extensions', count($extensions), 'slz')
				);
			}
		}
	...
	{
		$flash_id = 'slz_extensions_delete';

		if (!$this->can_install()) {
			SLZ_Flash_Messages::add(
				$flash_id,
				__('You are not allowed to delete extensions.', 'slz'),
				'error'
			);
			$this->js_redirect();
			return;
		...
		$extensions = array_fill_keys(array_map('trim', explode(',', SLZ_Request::GET('extension', ''))), array());

		{
			if (!class_exists('_SLZ_Extensions_Delete_Upgrader_Skin')) {
				slz_include_file_isolated(
					dirname(__FILE__) .'/includes/class--slz-extensions-delete-upgrader-skin.php'
				);
			}

			$skin = new _SLZ_Extensions_Delete_Upgrader_Skin(array(
				'title' => _n('Delete Extension', 'Delete Extensions', count($extensions), 'slz'),
			));
		}
		...
		do {
			...
			if ($_SERVER['REQUEST_METHOD'] === 'POST') {
				...
				slz_render_view(dirname(__FILE__) .'/views/delete-form.php', array(
	...
	public function uninstall_extensions(array $extensions, $opts = array())
	{
		...
		if (!$this->can_install()) {
			return new WP_Error(
				'access_denied',
				__('You have no permissions to uninstall extensions', 'slz')
			);
		}
		...
		while (!empty($extensions)) {
			...
			$wp_fs_extension_path = SLZ_WP_Filesystem::real_path_to_filesystem_path(
			...
			if ($verbose) {
				$verbose_message = sprintf(__('Deleting the "%s" extension...', 'slz'), $extension_title);
			...
			if (!$wp_filesystem->delete($wp_fs_extension_path, true, 'd')) {
				$result[$extension_name] = new WP_Error(
					...
					sprintf(__('Cannot delete the "%s" extension.', 'slz'), $extension_title)
				);
				$has_errors = true;
				...
				if ($verbose) {
					$verbose_message = sprintf(
						__('The %s extension has been successfully deleted.', 'slz'),
						$extension_title
					);
		...
		do_action('slz_extensions_uninstall', $result);
		...
		if (
			...
			if (
				...
				return new WP_Error(
					'uninstall_failed',
					_n('Cannot uninstall extension', 'Cannot uninstall extensions', count($extensions), 'slz')
				);
			}
		}
		...
		{
			update_option(
				slz()->extensions->_get_active_extensions_db_option_name(),
				array_diff_key(
					slz()->extensions->_get_db_active_extensions(),
					array_diff_key(
	...
	private function display_extension_page()
	{
		...
		$extension_name = trim(SLZ_Request::GET('extension', ''));

		$installed_extensions = $this->get_installed_extensions();

		$flash_id = 'slz_extension_page';

		{
			...
			do {
				if (empty($extension_name)) {
					$error = __('Extension not specified.', 'slz');
					break;
				}

				if (!isset($installed_extensions[$extension_name])) {
					$error = sprintf(__('Extension "%s" is not installed.', 'slz'), $this->get_extension_title($extension_name));
					break;
				}
			} while(false);

			if ($error) {
				SLZ_Flash_Messages::add($flash_id, $error, 'error');
				$this->js_redirect();
				return;
			}
		}
		...
		echo '<div class="wrap" id="slz-extension-page">';

		slz_render_view(dirname(__FILE__) .'/views/extension-page-header.php', array(
			...
			'tab'             => $tab,
			'is_supported'    =>
				slz()->theme->manifest->get('supported_extensions/'. $extension_name, false) !== false
				||
		...
		echo '<div id="slz-extension-tab-content">';
		{
			$method_data = array();
	...
	private function display_extension_settings_page($extension_name, $data)
	{
		if (!slz()->extensions->get($extension_name)) {
			return sprintf(
				__('Extension "%s" does not exist or is not active.', 'slz'),
				slz_htmlspecialchars($extension_name)
			);
		}

		$extension = slz()->extensions->get($extension_name);

		if (!$extension->get_settings_options()) {
			return sprintf(
				__('%s extension does not have settings.', 'slz'),
				$extension->manifest->get_name()
			);
		}

		echo '<div id="slz-extension-settings">';
	...
	private function display_extension_docs_page($extension_name, $data)
	{
		...
		if (!file_exists($docs_path)) {
			return __('Extension has no Install Instructions', 'slz');
		}

		echo slz()->backend->render_box(
			'slz-extension-docs',
			'',
			slz()->backend->render_options(array(
				'docs' => array(
					'label' => false,
					'type'  => 'html-full',
					'html'  => $this->get_markdown_parser()->text(
						slz_render_view($docs_path, array())
					),
				),
			))
		);
	...
	private function display_activate_page()
	{
		...
		do {
			if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
				$error = __('Invalid request method.', 'slz');
				break;
			}

			$nonce = $this->get_nonce('activate');

			if (!wp_verify_nonce(SLZ_Request::POST($nonce['name']), $nonce['action'])) {
				$error = __('Invalid nonce.', 'slz');
				break;
			}

			if (!isset($_GET['extension'])) {
				$error = __('No extension specified.', 'slz');
				break;
			}
	...
	public function activate_extensions(array $extensions, $cancel_on_error = false)
	{
		if (!$this->can_activate()) {
			return new WP_Error(
				'access_denied',
				__('You have no permissions to activate extensions', 'slz')
			);
		}
		...
		foreach ($extensions as $extension_name => $not_used_var) {
			if (!isset($installed_extensions[$extension_name])) {
				$result[$extension_name] = new WP_Error(
					...
					sprintf(__('Extension "%s" does not exist.', 'slz'), $this->get_extension_title($extension_name))
				);
				$has_errors = true;
		...
		if (
			...
			if (
				...
				return new WP_Error(
					'activation_failed',
					_n('Cannot activate extension', 'Cannot activate extensions', count($extensions), 'slz')
				);
			}
		}
		...
		update_option(
			slz()->extensions->_get_active_extensions_db_option_name(),
			array_merge(slz()->extensions->_get_db_active_extensions(), $extensions_for_activation)
		);
		...
		/**
		 * Prepare db wp option used to fire the 'slz_extensions_after_activation' action on next refresh
		 */
		{
			$db_wp_option_name = 'slz_extensions_activation';
		...
		do_action('slz_extensions_before_activation', $extensions_for_activation);
	...
	private function collect_required_extensions($ext_name, &$installed_extensions, &$collected)
	{
		...
		foreach (slz_akg('requirements/extensions', $installed_extensions[$ext_name]['manifest'], array()) as $req_ext_name => $req_ext_data) {
	...
	public function deactivate_extensions(array $extensions, $cancel_on_error = false)
	{
		if (!$this->can_activate()) {
			return new WP_Error(
				'access_denied',
				__('You have no permissions to deactivate extensions', 'slz')
			);
		}
		...
		if (
			...
			if (
				...
				return new WP_Error(
					'deactivation_failed',
					_n('Cannot deactivate extension', 'Cannot activate extensions', count($extensions), 'slz')
				);
			}
		}
		...
		$extensions_for_deactivation = array_merge($extensions_for_deactivation,
			array_fill_keys(
				array_keys(
					array_diff_key(
						$installed_extensions,
						$this->get_used_extensions($extensions_for_deactivation, array_keys(slz()->extensions->get_all()))
					)
				),
				array()
			)
		...
		/**
		 * Prepare db wp option used to fire the 'slz_extensions_after_deactivation' action on next refresh
		 */
		{
			$db_wp_option_name = 'slz_extensions_activation';
			$db_wp_option_value = get_option($db_wp_option_name, array(
		...
		do_action('slz_extensions_before_deactivation', $extensions_for_deactivation);
	...
	{
		/**
		 * @var SLZ_Extension $extension
		 */
		$extension = $data['data']['extension'];

		do_action('slz_extension_settings_form_render:'. $extension->get_name());

		echo slz_html_tag('input', array(
			'type'  => 'hidden',
			'name'  => 'slz_extension_name',
			'value' => $extension->get_name(),
		), true);

		echo slz()->backend->render_options(
			$extension->get_settings_options(),
			slz_get_db_ext_settings_option($extension->get_name())
		);
	...
	public function _extension_settings_form_validate($errors)
	{
		do {
			if (!current_user_can($this->can_activate())) {
				$errors[] = __('You are not allowed to save extensions settings.', 'slz');
				break;
			}

			$extension = slz()->extensions->get(SLZ_Request::POST('slz_extension_name'));

			if (!$extension) {
				$errors[] = __('Invalid extension.', 'slz');
				break;
			}

			if (!$extension->get_settings_options()) {
				$errors[] = __('Extension does not have settings options.', 'slz');
				break;
			}
		} while(false);
	...
	public function _extension_settings_form_save($data)
	{
		...
		$options_before_save = (array)slz_get_db_ext_settings_option($extension->get_name());

		slz_set_db_ext_settings_option(
			$extension->get_name(),
			null,
			array_merge(
				$options_before_save,
		...
		SLZ_Flash_Messages::add(
			'slz_extension_settings_saved',
			__('Extensions settings successfully saved.', 'slz'),
			'success'
		);

		$data['redirect'] = slz_current_url();

		do_action('slz_extension_settings_form_saved:'. $extension->get_name(), $options_before_save);
	...
	{
		$wp_error_id = 'slz_extension_download';

		// TODO: more checks for $data['download']
		if (empty($data['download'])) {
			return new WP_Error(
				$wp_error_id,
				sprintf(__('Extension "%s" has no download sources.', 'slz'), $this->get_extension_title($extension_name))
			);
		}
		...
		// create temporary directory
		{
			...
			if (!SLZ_WP_Filesystem::mkdir_recursive($wp_fs_tmp_dir)) {
				return new WP_Error(
					$wp_error_id,
					sprintf(__('Cannot create temporary directory: %s', 'slz'), $wp_fs_tmp_dir)
				);
			}
		}

		require_once dirname( __FILE__ ) . '/includes/download-source/class--slz-ext-download-source.php';
		require_once dirname( __FILE__ ) . '/includes/download-source/class--slz-ext-download-source-register.php';

		require_once dirname( __FILE__ ) . '/includes/download-source/types/init.php';

		$register = new _SLZ_Ext_Download_Source_Register(self::get_access_key()->get_key());

		/**
		 * Register download sources for extensions.
		 *
		 * Usage:
		 *   $download_source = new SLZ_Ext_Download_Source();
		 *   $register->register($download_source);
		 */
		do_action( 'slz_register_ext_download_sources', $register );
		...
		if (!$download_source) {
			return new WP_Error(
				'invalid_dl_source',
				sprintf(__('Invalid download source: %s', 'slz'), $data['download']['source'])
			);
		}
	...
	private function perform_zip_download(SLZ_Ext_Download_Source $download_source, array $opts, $wp_fs_tmp_dir)
	{
		$wp_error_id = 'slz_extension_download';
		...
		$unzip_result = unzip_file(
			SLZ_WP_Filesystem::filesystem_path_to_real_path($zip_path),
			$wp_fs_tmp_dir
		);
		...
		if (!$wp_filesystem->delete($zip_path, false, 'f')) {
			return new WP_Error(
				$wp_error_id,
				sprintf(__('Cannot remove the "%s" extension downloaded zip.', 'slz'), $this->get_extension_title($extension_name))
			);
		}
		...
		if (!$unzipped_dir_files) {
			return new WP_Error(
				$wp_error_id,
				__('Cannot access the unzipped directory files.', 'slz')
			);
		}
		...
		return new WP_Error(
			$wp_error_id,
			sprintf(__('The unzipped "%s" extension directory not found.', 'slz'), $this->get_extension_title($extension_name))
		);
	}
	...
	private function merge_extension($source_wp_fs_dir, $destination_wp_fs_dir)
	{
		...
		$wp_error_id = 'slz_extensions_merge';
		...
		// check source
		{
			...
			if ($source_files === false) {
				return new WP_Error(
					$wp_error_id,
					sprintf(__('Cannot read directory "%s".', 'slz'), $source_wp_fs_dir)
				);
			}
		...
		if ($wp_filesystem->exists($destination_wp_fs_dir)) {
			...
			if ($destination_files === false) {
				return new WP_Error(
					$wp_error_id,
					sprintf(__('Cannot read directory "%s".', 'slz'), $destination_wp_fs_dir)
				);
			}
			...
			if (!SLZ_WP_Filesystem::mkdir_recursive($destination_wp_fs_dir)) {
				return new WP_Error(
					$wp_error_id,
					sprintf(__('Cannot create the "%s" directory.', 'slz'), $destination_wp_fs_dir)
				);
			}
		}
		...
		if ($sub_extensions === false) {
			return new WP_Error(
				$wp_error_id,
				sprintf(__('Cannot read directory "%s".', 'slz'), $source_wp_fs_dir .'/extensions')
			);
		}
	...
	public function _filter_plugin_action_list($actions)
	{
		return array_merge(
			array(
				'slz-extensions' => slz_html_tag('a', array(
					'href' => $this->get_link(),
	...
	public function get_extension_title($extension_name)
	{
		...
		if (isset($installed_extensions[$extension_name])) {
			return slz_akg('name', $installed_extensions[$extension_name]['manifest'], slz_id_to_title($extension_name));
		}
		...
		return slz_id_to_title($extension_name);
	}
	...
	public function _action_enqueue_scripts()
	{
		...
		if ($this->is_extensions_page()) {
			wp_enqueue_style(
				...
				array(
					'slz',
					'slz-unycon', 'slz-font-awesome', // in case some extension has font-icon thumbnail
				),
				slz()->manifest->get_version()
			);
			wp_enqueue_script(
			...
			wp_localize_script('slz-extensions-page', '_slz_extensions_script_data', array(
				'link' => $this->get_link(),
			));

			/**
			 * this is needed for slz.soleModal design
		...
		if ($this->is_extension_page()) {
			...
			if (
				...
				&&
				slz()->extensions->get($extension_name)
				&&
				($extension_settings_options = slz()->extensions->get($extension_name)->get_settings_options())
			) {
				slz()->backend->enqueue_options_static($extension_settings_options);
			}
		}
	}
	...
	{
		$db_active_extensions = slz()->extensions->_get_db_active_extensions();
	...
	private function get_extensions_for_activation($extension_name)
	{
		...
		$wp_error_id = 'slz_ext_activation';

		if (!isset($installed_extensions[$extension_name])) {
			return new WP_Error($wp_error_id,
				sprintf(
					__('Cannot activate the %s extension because it is not installed. %s', 'slz'),
					$this->get_extension_title($extension_name),
	...
	public function _action_admin_notices() {
		...
		if (
			...
			, slz_html_tag('a', array('href' => $this->get_link() .'&sub-page=install&supported'),
				__('Install theme compatible extensions', 'slz'))
			, '</p></div>';
		}
	}
	...
	public function theme_available_extensions_copy() {
		...
		{
			$wpfs_tmp_dir = SLZ_WP_Filesystem::real_path_to_filesystem_path(
			...
			if (
				...
				return new WP_Error(
					'tmp_dir_rm_fail',
					sprintf(__('Temporary directory cannot be removed: %s', 'slz'), $wpfs_tmp_dir)
				);
			}

			if ( ! SLZ_WP_Filesystem::mkdir_recursive( $wpfs_tmp_dir ) ) {
				return new WP_Error(
					'tmp_dir_rm_fail',
					sprintf(__('Temporary directory cannot be created: %s', 'slz'), $wpfs_tmp_dir)
				);
			}
		}
		...
		$available_extensions = $this->get_available_extensions();
		$installed_extensions = $this->get_installed_extensions(true);
		$base_dir = slz_get_template_customizations_directory('/extensions');
		...
		foreach ($installed_extensions as $name => $ext) {
			...
			if ( ($rel_path = preg_replace('/^'. preg_quote($base_dir, '/') .'/', '', $ext['path'])) === $base_dir ) {
				return new WP_Error(
					'rel_path_failed',
					sprintf(__('Failed to extract relative directory from: %s', 'slz'), $ext['path'])
				);
			}

			if ( ($wpfs_path = SLZ_WP_Filesystem::real_path_to_filesystem_path($ext['path'])) === false) {
				return new WP_Error(
					'real_to_wpfs_filed',
					sprintf(__('Failed to extract relative directory from: %s', 'slz'), $ext['path'])
				);
			...
			if ( ! SLZ_WP_Filesystem::mkdir_recursive($wpfs_dest_dir) ) {
				return new WP_Error(
					'dest_dir_mk_fail',
					sprintf(__('Failed to create directory %s', 'slz'), $wpfs_dest_dir)
				);
			}
			...
			if ( is_wp_error( $copy_result = copy_dir($wpfs_path, $wpfs_dest_dir) ) ) {
				...
				return new WP_Error(
					'ext_copy_failed',
					sprintf( __('Failed to copy extension to %s', 'slz'), $wpfs_dest_dir )
				);
			}
		}
	}
	...
	public function theme_available_extensions_restore() {
		...
		if ( ! $wp_filesystem->exists(
			...
			return new WP_Error(
				'no_tmp_dir',
				sprintf(__('Temporary directory does not exist: %s', 'slz'), $wpfs_tmp_dir)
			);
		}
		 ...
		 * so slz_get_template_customizations_directory() also returns old path
		 */
		$theme_dir = wp_get_theme()->get_theme_root() .'/'. wp_get_theme()->get_template();

		if ( ! ($wpfs_base_dir = SLZ_WP_Filesystem::real_path_to_filesystem_path(
			$base_dir = $theme_dir . slz_get_framework_customizations_dir_rel_path('/extensions')
		) ) ) {
			return new WP_Error(
				'base_dir_to_wpfs_fail',
				sprintf( __('Cannot obtain WP Filesystem dir for %s', 'slz'), $base_dir )
			);
		}
	...
	public function _filter_theme_available_extensions_copy($result, $data) {
		if (
			...
			if ( is_wp_error( $copy_result = slz()->extensions->manager->theme_available_extensions_copy() ) ) {
				return $copy_result;
			}
		}
	...
	public function _action_theme_available_extensions_restore($instance, $data) {
		if (
			...
			slz()->extensions->manager->theme_available_extensions_restore();
		}
	}
}
