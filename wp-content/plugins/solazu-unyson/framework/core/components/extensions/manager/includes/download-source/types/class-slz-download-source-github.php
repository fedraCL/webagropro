...
class SLZ_Ext_Download_Source_Github extends SLZ_Ext_Download_Source
{
	...
	public function download(array $opts, $zip_path) {
		$wp_error_id = 'slz_ext_github_download_source';
		...
		if (empty($opts['user_repo'])) {
			return new WP_Error(
				$wp_error_id,
				sprintf(__('"%s" extension github source "user_repo" parameter is required', 'slz'), $extension_title)
			);
		}

		{
			$transient_name = 'slz_ext_mngr_gh_dl';
		...
		if (isset($cache[ $opts['user_repo'] ])) {
			...
			$response = $http->get(
				apply_filters('slz_github_api_url', 'https://api.github.com')
			...
			if ($response_code !== 200) {
				if ($response_code === 403) {
					...
					return new WP_Error(
						$wp_error_id,
						sprintf(
							__( 'Failed to access Github repository "%s" releases. (%s)', 'slz' ),
			...
			if (empty($release)) {
				return new WP_Error(
					$wp_error_id,
					sprintf(
						__('"%s" extension github repository "%s" has no releases.', 'slz'),
...

		{
			...
			if (($response_code = intval(wp_remote_retrieve_response_code($response))) !== 200) {
				if ($response_code) {
					return new WP_Error(
						$wp_error_id,
						sprintf( __( 'Cannot download the "%s" extension zip. (Response code: %d)', 'slz' ),
					...
					return new WP_Error(
						$wp_error_id,
						sprintf( __( 'Cannot download the "%s" extension zip. %s', 'slz' ),
							$extension_title,
					...
					return new WP_Error(
						$wp_error_id,
						sprintf( __( 'Cannot download the "%s" extension zip.', 'slz' ),
							$extension_title
						)
					);
				}
			...
			if (!$wp_filesystem->put_contents($zip_path, $response['body'])) {
				return new WP_Error(
					$wp_error_id,
					sprintf(__('Cannot save the "%s" extension zip.', 'slz'), $extension_title)
				);
			}
