 ...
 * @return null|mixed
 */
function slz_akg($keys, &$array_or_object, $default_value = null, $keys_delimiter = '/') {
	if (!is_array($keys)) {
	...
	if (isset($keys[0])) { // not used count() for performance reasons
		if ($is_object) {
			return slz_akg($keys, $array_or_object->{$key_or_property}, $default_value);
		} else {
			return slz_akg($keys, $array_or_object[$key_or_property], $default_value);
		}
	} else {
 ...
 * @param string $keys_delimiter
 */
function slz_aks($keys, $value, &$array_or_object, $keys_delimiter = '/') {
	if (!is_array($keys)) {
		$keys = explode($keys_delimiter, (string)$keys);
	}
	...
	if (isset($keys[0])) { // not used count() for performance reasons
		if ($is_object) {
			slz_aks($keys, $value, $array_or_object->{$key_or_property});
		} else {
			slz_aks($keys, $value, $array_or_object[$key_or_property]);
		}
	} else {
 ...
 * @param string $keys_delimiter
 */
function slz_aku($keys, &$array_or_object, $keys_delimiter = '/') {
	if (!is_array($keys)) {
		$keys = explode($keys_delimiter, (string)$keys);
	}
	...
	if (isset($keys[0])) { // not used count() for performance reasons
		if ($is_object) {
			slz_aku($keys, $array_or_object->{$key_or_property});
		} else {
			slz_aku($keys, $array_or_object[$key_or_property]);
		}
	} else {
 ...
 */
function slz_rand_md5() {
	return md5(time() .'-'. uniqid(rand(), true) .'-'. mt_rand(1, 1000));
}

function slz_unique_increment() {
	static $i = 0;
	return ++$i;
}
...
function slz_print($value) {
	...
	if ($first_time) {
		...
		div.slz_print_r_group div.slz_print_r {
			margin: 9px;
			border-width: 0;
		}
		</style>';
	...
	if (func_num_args() == 1) {
		echo '<div class="slz_print_r"><pre>';
		echo slz_htmlspecialchars(SLZ_Dumper::dump($value));
		echo '</pre></div>';
	} else {
		echo '<div class="slz_print_r_group">';
 ...
 */
function slz_html_tag($tag, $attr = array(), $end = false) {
	$html = '<'. $tag .' '. slz_attr_to_html($attr);
 ...
 */
function slz_attr_to_html(array $attr_array) {
	$html_attr = '';
	...
	foreach ($attr_array as $attr_name => $attr_val) {
		...
		$html_attr .= $attr_name .'="'. slz_htmlspecialchars($attr_val) .'" ';
	}
 ...
 */
function slz_stripslashes_deep_keys($value) {
	...
	if (is_array($value)) {
		if ($magic_quotes) {
			...
			foreach ($value as $key => $val) {
				$new_value[ is_string($key) ? stripslashes($key) : $key ] = slz_stripslashes_deep_keys($val);
			}
			$value = $new_value;
			unset($new_value);
		} else {
			$value = array_map('slz_stripslashes_deep_keys', $value);
		}
	} elseif (is_object($value)) {
		$vars = get_object_vars($value);
		foreach ($vars as $key=>$data) {
			$value->{$key} = slz_stripslashes_deep_keys($data);
		}
 ...
 */
function slz_addslashes_deep_keys($value) {
	static $magic_quotes = null;
	if ($magic_quotes === null) {
		$magic_quotes = get_magic_quotes_gpc();
	}
	...
	if (is_array($value)) {
		if ($magic_quotes) {
			...
			foreach ($value as $key=>$value) {
				$new_value[ is_string($key) ? addslashes($key) : $key ] = slz_addslashes_deep_keys($value);
			}
			$value = $new_value;
			unset($new_value);
		} else {
			$value = array_map('slz_addslashes_deep_keys', $value);
		}
	} elseif (is_object($value)) {
		$vars = get_object_vars($value);
		foreach ($vars as $key=>$data) {
			$value->{$key} = slz_addslashes_deep_keys($data);
		}
	} elseif (is_string($value)) {
 ...
 * @return bool
 */
function slz_current_screen_match(array $rules) {
 ...
 * @return string URI
 */
function slz_locate_theme_path_uri($rel_path) {
	try {
		return SLZ_File_Cache::get($cache_key = 'theme-uri:'. $rel_path);
 ...
 * @return string URI
 */
function slz_locate_theme_path($rel_path) {
	try {
		return SLZ_File_Cache::get($cache_key = 'theme-path:'. $rel_path);
	} catch (SLZ_File_Cache_Not_Found_Exception $e) {
		if (is_child_theme() && file_exists(get_stylesheet_directory() . $rel_path)) {
 ...
 * @return string HTML
 */
function slz_render_view($file_path, $view_variables = array(), $return = true) {
 ...
 * @return array
 */
function slz_get_variables_from_file($file_path, array $_extract_variables, array $_set_variables = array()) {
 ...
 */
function slz_include_file_isolated($file_path, $once = false) {
 ...
 */
function slz_extract_only_options(array $options) {
	$collected = array();

	slz_collect_options($collected, $options);
 ...
 * Use slz_collect_options()
 */
function slz_collect_first_level_options(&$collected, &$options) {
	if (empty($options)) {
		return;
	}
	...
	foreach ($options as $option_id => &$option) {
		if (isset($option['options'])) {
			 ...
			 * )
			 */
			slz_collect_first_level_options($collected, $option);
 ...
 */
function slz_collect_options(&$result, &$options, $settings = array(), $_recursion_data = array()) {
	...
	if (empty($_recursion_data)) {
		if (is_null($access_key)) {
			$access_key = new SLZ_Access_Key('slz_collect_options');
		}
		...
		&&
		($_recursion_data['access_key'] instanceof SLZ_Access_Key)
		&&
		($_recursion_data['access_key']->get_key() === 'slz_collect_options')
	)) {
	...
	foreach ($options as $option_id => &$option) {
		if (isset($option['options'])) { // this is a container
			 ...
			 * )
			 */
			slz_collect_options($result, $option, $settings, $_recursion_data);
 ...
 * @return array Values
 */
function slz_get_options_values_from_input(array $options, $input_array = null) {
	if (!is_array($input_array)) {
		$input_array = SLZ_Request::POST(slz()->backend->get_options_name_attr_prefix());
	}

	$values = array();

	foreach (slz_extract_only_options($options) as $id => $option) {
		$values[$id] = slz()->backend->option_type($option['type'])->get_value_from_input(
			$option,
 ...
 * @return mixed
 */
function slz_html_attr_name_to_array_multi_key($attr_name, $set_mode = false) {
	if ($set_mode) {
		/**
 ...
 * @return array
 */
function slz_prepare_option_value($value) {
	if (empty($value)) {
		return $value;
	}
 ...
 */
function slz_is_real_post_save($post_id) {
	return !(
 ...
 */
function slz_get_google_fonts() {
	$cache_key = 'slz_google_fonts';

	try {
		return SLZ_Cache::get($cache_key);
	} catch (SLZ_Cache_Not_Found_Exception $e) {
		$g_fonts = json_decode(slz_get_google_fonts_v2(), true);
		$old_fonts = include(dirname(__FILE__) .'/slz-google-fonts.json.php');
		$fonts = array();
		...
		$fonts = apply_filters('slz_google_fonts', $fonts);

		SLZ_Cache::set($cache_key, $fonts);
 ...
 */
function slz_get_google_fonts_v2() {
	$saved_data = get_option( 'slz_google_fonts', false );
	...
	if (
		...
		$response = wp_remote_get( apply_filters( 'slz_googleapis_webfonts_url', 'http://google-webfonts-cache.unyson.io/v1/webfonts' ) );
		...
		if (
			...
			update_option( 'slz_google_fonts', array(
				'last_update' => time(),
				'fonts'       => $body
			), false );
...
function slz_is_valid_domain_name($domain_name) {
 ...
 * @return string
 */
function slz_htmlspecialchars($string) {
 ...
 */
function slz_current_user_can($capabilities, $default_value = null)
{
 ...
 * @return string
 */
function slz_human_time($seconds)  {
	...
	if ($translations === null) {
		$translations = array(
			...
			'minute'    => __('minute', 'slz'),
			'minutes'   => __('minutes', 'slz'),

			'second'    => __('second', 'slz'),
			'seconds'   => __('seconds', 'slz'),
		);
	}
 ...
 * @since 2.4.17
 */
function slz_human_bytes($bytes, $precision = 2) {
	$kilobyte = 1024;
 ...
 */
function slz_dirname_to_classname($dirname) {
 ...
 * @return bool|string
 */
function slz_oembed_get($url, $args = array()) {
 ...
 */
function slz_secure_rand($length)
{
 ...
 */
function slz_ext($extension_name) {
	return slz()->extensions->get($extension_name);
}

/*
 * Return URI without scheme
 */
function slz_get_url_without_scheme( $url ) {
 ...
 */
function slz_read_file_by_uri($file_uri) {
	static $base = null;
 ...
 */
function slz_make_stylesheet_portable($href, $contents = null) {
	if (is_null($contents)) {
		$contents = slz_read_file_by_uri($href);
 ...
 * @return array|bool
 */
function slz_get_image_sizes( $size = '' ) {
 ...
 * @return string
 */
function slz_string_to_icon_html($icon, array $attributes = array()) {
	if (
 ...
 * @since 2.4.10
 */
function slz_get_json_last_error_message() {
	switch (function_exists('json_last_error') ? json_last_error() : -1) {
		case JSON_ERROR_NONE:
			return null; // __('No errors', 'slz');
			break;
		case JSON_ERROR_DEPTH:
			return __('Maximum stack depth exceeded', 'slz');
			break;
		case JSON_ERROR_STATE_MISMATCH:
			return __('Underflow or the modes mismatch', 'slz');
			break;
		case JSON_ERROR_CTRL_CHAR:
			return __('Unexpected control character found', 'slz');
			break;
		case JSON_ERROR_SYNTAX:
			return __('Syntax error, malformed JSON', 'slz');
			break;
		case JSON_ERROR_UTF8:
			return __('Malformed UTF-8 characters, possibly incorrectly encoded', 'slz');
			break;
		default:
			return __('Unknown error', 'slz');
			break;
	}
}
 ...
 * @return array
 */
function slz_get_mime_type_by_ext( $type = array() ) {
	$result = array();
 ...
 * @return array
 */
function slz_multi_ext2type( $ext_array = array() ) {
