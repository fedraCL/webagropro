 ...
 * it will handle all clever logic with default values, multikeys and processing options slz-storage parameter
 * @since 2.5.9
 */
abstract class SLZ_Db_Options_Model {
	/**
	 ...
	 * @see slz_db_option_storage_type()
	 */
	abstract protected function get_slz_storage_params($item_id, array $extra_data = array());
	...
	final public function get_main_cache_key() {
		return 'slz-options-model:'. $this->get_id();
	}
	...
	final public function get( $item_id = null, $option_id = null, $default_value = null, array $extra_data = array() ) {
		...
		try {
			...
			$values = SLZ_Cache::get($cache_key_values = $this->get_cache_key('values', $item_id, $extra_data));
		...
		if ( ! is_null($default_value) ) {
			if ( empty( $option_id ) ) {
				...
				if ( is_null( $sub_keys ) ) {
					...
					if ( is_null( slz_akg( $sub_keys, $values[ $option_id ] ) ) ) {
						return $default_value;
					}
				}
			}
		...
		try {
			$options = SLZ_Cache::get( $cache_key = $this->get_cache_key('options', $item_id, $extra_data));
		} catch (SLZ_Cache_Not_Found_Exception $e) {
			SLZ_Cache::set($cache_key, array()); // prevent recursion
			SLZ_Cache::set($cache_key, $options = slz_extract_only_options($this->get_options($item_id, $extra_data)));
		}
		...
		if ($options) {
			try {
				 ...
				 */
				SLZ_Cache::set($cache_key_values_processed, true);

				// Complete missing db values with default values from options array
				$values = array_merge(
					slz_get_options_values_from_input($options, array()),
					$values
				);

				foreach ($options as $id => $option) {
					$values[$id] = slz()->backend->option_type($option['type'])->storage_load(
						$id,
						$option,
						isset($values[$id]) ? $values[$id] : null,
						$this->get_slz_storage_params($item_id, $extra_data)
					);
				}

				SLZ_Cache::set($cache_key_values, $values);
			}
		}
		...
		if (empty($option_id)) {
			...
			if (is_null($sub_keys)) {
				...
				return slz_akg($sub_keys, $values[$option_id], $default_value);
			}
		}
	}

	final public function set( $item_id = null, $option_id = null, $value, array $extra_data = array() ) {
		SLZ_Cache::del($cache_key_values = $this->get_cache_key('values', $item_id, $extra_data));
		SLZ_Cache::del($cache_key_values_processed = $this->get_cache_key('values:processed', $item_id, $extra_data));
		...
		if ($option_id) {
			...
			if ($sub_keys) { // update sub_key in old_value and use the entire value
				$new_value = $old_value;
				slz_aks($sub_keys, $value, $new_value);
				$value = $new_value;
				unset($new_value);

				$old_value = slz_akg($sub_keys, $old_value);
			}

			if (isset($options[$option_id])) {
				$value = slz()->backend->option_type($options[$option_id]['type'])->storage_save(
					$option_id,
					$options[$option_id],
					$value,
					$this->get_slz_storage_params($item_id, $extra_data)
			...
			foreach ($value as $_option_id => $_option_value) {
				if (isset($options[$_option_id])) {
					$value[$_option_id] = slz()->backend->option_type($options[$_option_id]['type'])->storage_save(
						$_option_id,
						$options[$_option_id],
						$_option_value,
						$this->get_slz_storage_params($item_id, $extra_data)
		...
		SLZ_Cache::del($cache_key_values); // fixes https://github.com/ThemeFuse/Unyson/issues/1538
		SLZ_Cache::del($cache_key_values_processed);
