...
/**
 * Persistent cache saved in uploads/slz-file-cache.php
 *
 * It is reset when:
 ...
 * Usage:
 * try {
 *     return SLZ_File_Cache::get($cache_key = '...'. $rel_path);
 * } catch (SLZ_File_Cache_Not_Found_Exception $e) {
 *     $result = ...;
 *
 *     SLZ_File_Cache::set($cache_key, $result);
 *
 *     return $result; // IMPORTANT: Do not use SLZ_File_Cache::get($cache_key) again
 * }
 *
 * @since 2.6.0
 */
class SLZ_File_Cache {
	/**
	 * @var int
	 */
	...
	private static function get_path() {
		if (is_null(self::$path)) {
			...
			self::$path = slz_fix_path(self::$path['basedir']) . '/slz/file-cache.php';
		}
	...
	public static function _init() {
		...
		if ( current_user_can('manage_options') ) {
			 ...
			 */
			foreach (array(
				'slz_extensions_before_activation' => true,
				'slz_extensions_after_activation' => true,
				'slz_extensions_before_deactivation' => true,
				'slz_extensions_after_deactivation' => true,
				'slz_extensions_install' => true,
				'slz_extensions_uninstall' => true,
	 ...
	 * @return mixed
	 * @throws SLZ_File_Cache_Not_Found_Exception
	 */
	public static function get($key) {
		if ( ! self::load() ) {
			throw new SLZ_File_Cache_Not_Found_Exception();
		}
...
class SLZ_File_Cache_Not_Found_Exception extends Exception {}

SLZ_File_Cache::_init();
