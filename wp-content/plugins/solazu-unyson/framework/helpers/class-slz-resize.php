...
if ( ! function_exists( 'slz_resize' ) ) {
	function slz_resize( $url, $width = false, $height = false, $crop = false ) {
		$slz_resize = SLZ_Resize::getInstance();
		$response  = $slz_resize->process( $url, $width, $height, $crop );

		return ( ! is_wp_error( $response ) && ! empty( $response['src'] ) ) ? $response['src'] : $url;
	}
}

if ( ! function_exists( 'slz_delete_resized_thumbnails' ) ) {
	function slz_delete_resized_thumbnails( $id ) {
	...
	add_action( 'delete_attachment', 'slz_delete_resized_thumbnails' );
}
