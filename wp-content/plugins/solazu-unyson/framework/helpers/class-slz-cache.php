 ...
 *  try {
 *      $value = SLZ_Cache::get('some/key');
 *  } catch(SLZ_Cache_Not_Found_Exception $e) {
 *      $value = get_value_from_somewhere();
 *
 *      SLZ_Cache::set('some/key', $value);
 *
 *      // (!) after set, do not do this:
 *      $value = SLZ_Cache::get('some/key');
 *      // because there is no guaranty that SLZ_Cache::set('some/key', $value); succeeded
...
class SLZ_Cache
{
	...
	{
		self::$not_found_value = new SLZ_Cache_Not_Found_Exception();
	...
	public static function set($keys, $value, $keys_delimiter = '/')
	{
		...
		slz_aks($keys, $value, self::$cache, $keys_delimiter);
	...
	{
		slz_aku($keys, self::$cache, $keys_delimiter);
	 ...
	 * @param $keys_delimiter
	 * @return mixed
	 * @throws SLZ_Cache_Not_Found_Exception
	 */
	public static function get($keys, $keys_delimiter = '/')
	{
		...
		$value = slz_akg($keys, self::$cache, self::$not_found_value, $keys_delimiter);
		...
		if ($value === self::$not_found_value) {
			...
			throw new SLZ_Cache_Not_Found_Exception();
		} else {
			++self::$hits;
	...
	/**
	 * Debug information
	 * <?php add_action('admin_footer', function(){ SLZ_Cache::stats(); });
	 * @since 2.4.17
	 */
	public static function stats() {
		...
		echo '<strong>PHP Memory Peak Usage:</strong> '. slz_human_bytes(memory_get_peak_usage(false)) .'<br />';
		echo '</p>';
		echo '<ul>';
...
class SLZ_Cache_Not_Found_Exception extends Exception {}
