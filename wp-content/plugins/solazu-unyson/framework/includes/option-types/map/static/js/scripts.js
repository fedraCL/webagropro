...
(function($, _, slze, localized){
	jQuery( document ).ready( function( ) {
		...
		function slz_option_map_initialize( data ) {

			if( typeof google === 'undefined' ){
				data.find( '.slz-option-map-inputs').attr( 'readonly', 'readonly' );
				return;
			}
			...
			var option = {
				...
				toggles : {
					expand : data.find('.slz-option-maps-expand'),
					reset : data.find('.slz-option-maps-close')
				},
				tabs    : {
					first   : data.find('.slz-option-maps-tab.first'),
					second  : data.find('.slz-option-maps-tab.second')
				},
		...
		slze.on('slz:options:init', function (data) {
			...
			var obj = data.$elements.find('.slz-option-type-map:not(.initialized)');
				...
				if (pendingInit.length) { // already in process of loading the script
					...
					$.ajax({
						...
						$.each(pendingInit, function(i, obj){
							obj.each(function(){
								slz_option_map_initialize($(this));
							});
						});
						pendingInit = [];
					}).fail(function(){
...
})(jQuery, _, slzEvents, _slz_option_type_map);
