...
class SLZ_Option_Type_Addable_Popup extends SLZ_Option_Type
{
	...
	protected function _enqueue_static($id, $option, $data)
	{
		...
		if ($enqueue) {
			wp_enqueue_style(
				'slz-option-' . $option_type,
				slz_get_framework_directory_uri('/includes/option-types/' . $option_type . '/static/css/styles.css'),
				array('slz'),
			...
			wp_enqueue_script(
				'slz-option-' . $option_type,
				slz_get_framework_directory_uri('/includes/option-types/' . $option_type . '/static/js/scripts.js'),
	...
	protected function _render($id, $option, $data)
	{
		...
		$option['attr']['data-for-js'] =
			 ...
			 * Do this trick because {{ is not escaped/encoded by slz_htmlspecialchars()
		...
		return slz_render_view(
			slz_get_framework_directory('/includes/option-types/addable-popup/view.php'),
	...
	protected function _get_value_from_input($option, $input_value)
	{
		 ...
		 */
		return apply_filters('slz:option-type:addable-popup:value-from-input', $values, $option);
	}
...
SLZ_Option_Type::register('SLZ_Option_Type_Addable_Popup');

class SLZ_Option_Type_Addable_Popup_Full extends SLZ_Option_Type_Addable_Popup
{
	public function get_type()
	{
	...
	protected function _render($id, $option, $data)
	{
		...
		$option['attr']['class'] .= ' slz-option-type-addable-popup';

		return parent::_render($id, $option, $data);
	}
}

SLZ_Option_Type::register('SLZ_Option_Type_Addable_Popup_Full');
