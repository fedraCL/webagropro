...
class SLZ_Option_Type_Datetime_Picker extends SLZ_Option_Type {
	...
	protected function _render( $id, $option, $data ) {
		...
		$wrapper_attr['data-min-date'] = slz_akg('datetime-picker/minDate', $option, false );
		$wrapper_attr['data-max-date'] = slz_akg('datetime-picker/maxDate', $option, false );
		...
		echo '<div ' . slz_attr_to_html($wrapper_attr) .' >';
		echo slz()->backend->option_type( 'text' )->render( $id, $option, $data );
		echo '</div>';
	}
	...
	protected function _enqueue_static($id, $option, $data) {
		//plugin styles & js
		{
			$css_lib_uri        = slz_get_framework_directory_uri('/includes/option-types/datetime-picker/static/css/jquery.datetimepicker.css');
			$js_lib_uri         = slz_get_framework_directory_uri('/includes/option-types/datetime-picker/static/js/jquery.datetimepicker.js');
		}

		//framework styles & js
		{
			$css_main_uri    = slz_get_framework_directory_uri('/includes/option-types/datetime-picker/static/css/style.css');
			$js_main_uri     = slz_get_framework_directory_uri('/includes/option-types/datetime-picker/static/js/script.js');
		}

		wp_enqueue_style( 'slz-option-datetime-picker-lib-css', $css_lib_uri );
		wp_enqueue_style( 'slz-option-datetime-picker-main-css', $css_main_uri );
		wp_enqueue_script( 'slz-moment' );
		wp_enqueue_script( 'slz-option-datetime-picker-lib-js', $js_lib_uri, array('jquery', 'slz-moment'), false, true );
		wp_enqueue_script( 'slz-option-datetime-picker-main-js', $js_main_uri, array('jquery', 'slz-option-datetime-picker-lib-js', 'slz-events' ), false, true );
	...
	protected function _get_value_from_input($option, $input_value)
	{
		...
		if ( isset($option['datetime-picker']['format']) && !empty($option['datetime-picker']['format']) ){
			if ($this->_slz_validate_date_format($input_value, $option['datetime-picker']['format'], $extra_formats ) === false) {
				return $option['value'];
			}
		}
	...
	private function _slz_validate_date_format($date, $format = 'Y/m/d H:i', $extra_formats = array())
	{
...
SLZ_Option_Type::register('SLZ_Option_Type_Datetime_Picker');
