...
class SLZ_Option_Type_Multi_Upload extends SLZ_Option_Type
{
	...
	protected function _enqueue_static($id, $option, $data)
	{
		...
		{
			wp_enqueue_style(
				'slz-option-type-'. $this->get_type() . '-modal',
				$this->css_uri . 'modal.css',
				array(),
				slz()->manifest->get_version()
			);
			wp_enqueue_style(
				'slz-option-type-'. $this->get_type() . '-images-only',
			...
			wp_enqueue_script(
				...
				array('jquery', 'slz-events', 'underscore', 'jquery-ui-sortable'),
				slz()->manifest->get_version(),
				true
			);
		}

		{
			...
			wp_enqueue_style(
				'slz-option-type-'. $this->get_type() . '-any-files',
	...
	protected function _render($id, $option, $data)
	{
		...
		if ( ! empty( $option['files_ext'] ) ) {
			...
			$mime_types = slz_get_mime_type_by_ext( $ext_files );
			$collector  = array(
				'ext_files'  => $ext_files,
			...
			$check_file_types      = slz_multi_ext2type( $ext_files );
	...
	private function render_images_only($input_attr, $wrapper_attr, $l10n)
	{
		$l10n = array_merge(
			array(
				'button_add'    => __('Add Images', 'slz'), // TODO: add context ?
				'button_edit'   => __('Edit', 'slz'), // TODO: add context ?
				'modal_title'   => __('Select Images', 'slz'),
			),
			$l10n
		);
		...
		return slz_render_view($this->views_path . 'images-only.php', array(
	...
	private function render_any_files($input_attr, $wrapper_attr, $l10n)
	{
		$l10n = array_merge(
			array(
				'button_add'  => __('Upload', 'slz'), // TODO: add context ?
				'button_edit' => __('Edit', 'slz'), // TODO: add context ?
				'files_one'   => __('1 File', 'slz'), // TODO: maybe add context ?
				'files_more'  => __('%u Files', 'slz'), // TODO: maybe add context ?
			),
			$l10n
		);
		$wrapper_attr = array_merge($wrapper_attr, array(
		...
		return slz_render_view($this->views_path . 'any-files.php', array(
			'wrapper_attr' => $wrapper_attr,
			'input_attr'   => $input_attr,
			'is_empty'     => $is_empty,
			'l10n'         => $l10n
...
SLZ_Option_Type::register('SLZ_Option_Type_Multi_Upload');
