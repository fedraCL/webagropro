jQuery(function($){
	...
	var helpers = {
		optionClass: 'slz-option-type-rgba-color-picker',
		eventNamespace: '.slzOptionTypeRgbaColorPicker',
		hexColorRegex: /^#[a-f0-9]{3}([a-f0-9]{3})?$/i,
		localized: window._slz_option_type_rgba_color_picker_localized,
		increment: 0,
	...
	slzEvents.on('slz:options:init', function (data) {
		data.$elements.find('input.'+ helpers.optionClass +':not(.initialized)').each(function () {
			...
			$input.one('focus', function(){
				...
				$input.iris({
					...
					change: function (event, ui) {
						...
						changeTimeoutId = setTimeout(function(){
							$input.trigger('slz:option-type:rgba-color-picker:change', {
								$element: $input,
				 ...
				 */
				{
					$input.parent().attr('id', 'slz-rgba-color-picker-r-'+ (++helpers.increment));
