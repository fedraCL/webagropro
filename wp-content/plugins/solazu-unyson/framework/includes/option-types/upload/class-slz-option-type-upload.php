...
class SLZ_Option_Type_Upload extends SLZ_Option_Type
{
	private $views_path;
	private $js_uri;
	private $css_uri;
	...
	protected function _enqueue_static($id, $option, $data)
	{
...

		{
			...
			wp_enqueue_script(
				...
				array('jquery', 'slz-events', 'underscore'),
				slz()->manifest->get_version(),
				true
			);
		}
	...
	private function render_images_only($input_attr, $wrapper_attr, $l10n)
	{
		$l10n = array_merge(
			array(
				'button_add'    => __('Add Image', 'slz'),
				'button_edit'   => __('Edit', 'slz'),
				'modal_title'   => __('Select Image', 'slz'),
			),
			$l10n
		);
	...
	protected function _get_value_from_input($option, $input_value)
	{
		if (empty($input_value)) {
			...
			// [slz_get_options_values_from_input](slz-get-options-values-from-input) 
...
SLZ_Option_Type::register('SLZ_Option_Type_Upload');
