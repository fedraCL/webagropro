...
require_once dirname(__FILE__) . '/includes/class-slz-wp-editor-settings.php';

class SLZ_Option_Type_Wp_Editor extends SLZ_Option_Type {
	...
	protected function get_default_shortcodes_list() {
		$editor_shortcodes = slz_ext('wp-shortcodes');
	...
	protected function _render( $id, $option, $data ) {
		...
		$editor_manager = new SLZ_WP_Editor_Manager($id, $option, $data);
	...
	protected function _enqueue_static( $id, $option, $data ) {
		 ...
		 */
		{
			wp_enqueue_style(
				 ...
				 */
				'slz-option-type-' . $this->get_type() .'-dashicons',
			...
			wp_enqueue_style(
				 ...
				 * without prefix it won't enqueue
				 */
				'slz-option-type-' . $this->get_type() .'-editor-buttons',
				includes_url('/css/editor.min.css'),
				array('dashicons', 'slz-unycon'),
				slz()->manifest->get_version()
			);
		}

		$uri = slz_get_framework_directory_uri(
		...
		wp_enqueue_script(
			...
			$uri . '/scripts.js',
			array('jquery', 'slz-events', 'editor', 'slz'),
			slz()->manifest->get_version(),
			true
		);
		...
		do_action('slz:option-type:wp-editor:enqueue-scripts');
	}
...
SLZ_Option_Type::register( 'SLZ_Option_Type_Wp_Editor' );
