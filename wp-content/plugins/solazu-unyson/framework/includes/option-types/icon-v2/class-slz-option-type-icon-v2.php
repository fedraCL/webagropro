...
require_once dirname(__FILE__) . '/includes/class-slz-icon-v2-packs-loader.php';
require_once dirname(__FILE__) . '/includes/class-slz-icon-v2-favorites.php';

class SLZ_Option_Type_Icon_v2 extends SLZ_Option_Type
{
	...
	public function _init()
	{
		 ...
		 *
		 * slz()->backend->option_type('icon-v2')->packs_loader->enqueue_frontend_css()
		 */
		$this->packs_loader = new SLZ_Icon_V2_Packs_Loader();

		if (! is_admin()) { return; }

		$this->favorites = new SLZ_Icon_V2_Favorites_Manager();
	...
	protected function _enqueue_static($id, $option, $data)
	{
		...
		$static_URI = slz_get_framework_directory_uri(
		...
		wp_enqueue_script(
			'slz-option-type-'. $this->get_type() .'-backend-previews',
			$static_URI . 'js/render-icon-previews.js',
			array('jquery', 'slz', 'slz-events', 'slz-selectize'),
			slz()->manifest->get_version()
		);

		wp_enqueue_script(
			'slz-option-type-'. $this->get_type() .'-backend-picker-v2',
		...
		wp_enqueue_style(
			'slz-option-type-'. $this->get_type() .'-backend-picker',
		...
		wp_localize_script(
			...
			'slz_icon_v2_data',
			array(
				'edit_icon_label' => __('Change Icon', 'slz'),
				'add_icon_label' => __('Add Icon', 'slz')
			)
		);
	}

	public function load_templates() {
		echo slz_render_view(
...
$favorites = new SLZ_Icon_V2_Favorites_Manager();
$favorites->attach_ajax_actions();

SLZ_Option_Type::register( 'SLZ_Option_Type_Icon_v2' );
