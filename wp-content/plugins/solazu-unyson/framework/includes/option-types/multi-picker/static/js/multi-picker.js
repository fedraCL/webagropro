(function($, slze) {
	var init = function() {
		var $this = $(this),
			...
			flows = {
				...
				'icon-v2': function () {
					var iconV2Selector = '.slz-option-type-icon-v2 > input';
		...
		if (!pickerType) {
			...
			if (flows[pickerType]) {
				...
				var eventName = 'slz:option-type:multi-picker:init:'+ pickerType;

				if (slze.hasListeners(eventName)) {
					slze.trigger(eventName, {
	...
	slze.on('slz:options:init', function(data) {
		data.$elements
			.find('.slz-option-type-multi-picker:not(.slz-option-initialized)').each(init)
