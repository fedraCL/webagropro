...
class Slz_Option_Type_Image_Picker extends SLZ_Option_Type
{
	...
	{
		wp_enqueue_script(
			'slz-option-' . $this->get_type() . '-image-picker',
			slz_get_framework_directory_uri('/includes/option-types/' . $this->get_type() . '/static/js/image-picker/image-picker.js'),
			array(),
...

		{
			...
			foreach ($option['choices'] as $key => $choice) {
				...
				if (!empty($choice['large'])) {
					...
					$pre_load_images_html .= slz_html_tag('img', array('src' => $choice['large']['src']));
				}
				...
				$html .= slz_html_tag('option', $attr, slz_htmlspecialchars(isset($choice['label']) ? $choice['label'] : ''));
			}

			$html .= '</select>';
		}

		return slz_html_tag('div', $wrapper_attr,
...
SLZ_Option_Type::register('SLZ_Option_Type_Image_Picker');
