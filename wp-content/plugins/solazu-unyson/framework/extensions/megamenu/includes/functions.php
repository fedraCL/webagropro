 ...
 * @return mixed
 */
function _slz_ext_mega_menu_meta($post, $key, $default = null, $write = false) {
	static $meta = array();
	...
	if ($write) {
		...
		slz_update_post_meta($post_id, 'mega-menu', $meta[$post_id]);
		return null;
	}
 ...
 * @internal
 */
function _slz_ext_mega_menu_admin_input_name($post, $key) {
	$post_id = is_object($post) ? $post->ID : $post;
 ...
 * @internal
 */
function _slz_ext_mega_menu_admin_input_POST_values($post) {
	$post_id = is_object($post) ? $post->ID : $post;

	return (array)slz_akg('mega-menu/'. $post_id, $_POST);
}
