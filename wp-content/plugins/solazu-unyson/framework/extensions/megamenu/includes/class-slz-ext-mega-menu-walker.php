...
class SLZ_Ext_Mega_Menu_Walker extends Walker_Nav_Menu
{
	/**
...
# BEGIN - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		...
		$item_output = slz_ext('megamenu')->render_str('item-link', compact('item', 'attributes', 'title', 'args', 'depth'));
# END - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# BEGIN - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
				if ($depth == 0 && slz_ext_mega_menu_get_meta($id, 'enabled') && slz_ext_mega_menu_get_meta($child, 'new-row')) {
...
# END - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
				if ( !isset($newlevel) ) {
					$newlevel = true;
# BEGIN - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
					if (!isset($mega_menu_container) && $depth == 0 && slz_ext_mega_menu_get_meta($id, 'enabled')) {
						$mega_menu_container = apply_filters('slz_ext_mega_menu_container', array(
							'tag'  => 'div',
						...
						$output .= '<'. $mega_menu_container['tag'] .' '. slz_attr_to_html($mega_menu_container['attr']) .'>';
					}
					...
					$classes = apply_filters('slz_ext_mega_menu_start_lvl_classes', $classes, array(
						'element' => $element,
						'children_elements' => $children_elements,
						'max_depth' => $max_depth,
						'depth' => $depth,
...
# END - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	protected function sub_menu_contains_icons($element, $children_elements) {
		...
		$id = $element->$id_field;
		foreach ($children_elements[$id] as $child) {
			if (slz_ext_mega_menu_get_meta($child, 'icon')) {
				return true;
			}
		}
		return false;
	...
	protected function row_contains_icons($row, $first_column, $children_elements) {
		...
		while (true) {
			...
			if ($child === false || slz_ext_mega_menu_get_meta($child, 'new-row')) {
				break;
			}
		}
 ...
 * @deprecated
 */
class SLZ_Theme_Menu_Walker extends SLZ_Ext_Mega_Menu_Walker {
