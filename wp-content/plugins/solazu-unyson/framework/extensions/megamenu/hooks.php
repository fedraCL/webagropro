 ...
 * @internal
 */
function _filter_slz_ext_mega_menu_wp_nav_menu_args($args) {
	...
	$args['walker'] = new SLZ_Ext_Mega_Menu_Walker();

	return $args;
}
add_filter('wp_nav_menu_args', '_filter_slz_ext_mega_menu_wp_nav_menu_args');

/**
 * Just for removing SLZ_Ext_Mega_Menu_Walker set in the previous
 * filter when the fallback menu is in action.
 * @param array $args
 * @return array
 * @internal
 */
function _filter_slz_ext_mega_menu_wp_page_menu_args($args) {
	if ($args['walker'] instanceof SLZ_Ext_Mega_Menu_Walker) {
		$args['walker'] = '';
	}

	return $args;
}
add_filter('wp_page_menu_args', '_filter_slz_ext_mega_menu_wp_page_menu_args');
 ...
 * @internal
 */
function _filter_slz_ext_mega_menu_wp_nav_menu_objects($sorted_menu_items, $args) {
	...
	foreach ($sorted_menu_items as $item) {
		if ($item->menu_item_parent == 0 && slz_ext_mega_menu_get_meta($item, 'enabled')) {
	...
	foreach ($sorted_menu_items as $item) {
		...
		if (slz_ext_mega_menu_get_meta($item, 'icon')) {
...
add_filter('wp_nav_menu_objects', '_filter_slz_ext_mega_menu_wp_nav_menu_objects', 10, 2);
 ...
 * @internal
 */
function _filter_slz_ext_mega_menu_walker_nav_menu_start_el($item_output, $item, $depth, $args) {
	if (!slz_ext_mega_menu_is_mm_item($item)) {
		return $item_output;
	}
	...
	if ($depth > 0 && slz_ext_mega_menu_get_meta($item, 'title-off')) {
		$item_output = '';
	}
...
add_filter('walker_nav_menu_start_el', '_filter_slz_ext_mega_menu_walker_nav_menu_start_el', 10, 4);
