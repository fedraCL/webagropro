...
class SLZ_Post
{
	...
	/**
	 * Gets the path at which the post is located
	...
	/**
	 * Gets the uri at which the post is located
	...
	final public function locate_path($rel_path = '')
	{
		if (class_exists('SLZ_File_Cache')) {
			try {
				return SLZ_File_Cache::get($cache_key = 'ext:shcd:post-' . $this->tag .':path:'. $rel_path);
	...
	final public function locate_URI($rel_path = '')
	{
		if (class_exists('SLZ_File_Cache')) {
			try {
				return SLZ_File_Cache::get( $cache_key = 'ext:shcd:post-' . $this->tag . ':uri:' . $rel_path );
			} catch ( SLZ_File_Cache_Not_Found_Exception $e ) {
				$result = false;
	...
	public function get_options()
	{
		...
		return apply_filters('slz_post_get_options', $this->options, $this->tag);
	}
	...
	public function render( $echo = true )
	{
		...
		if (!$view_file) {
			trigger_error(
				sprintf(__('No default view (views/view.php) found for post: %s', 'slz'), $this->tag),
				E_USER_ERROR
			);
		}
		...
		$view_extra = apply_filters('slz_post_render_view', array(
			'before' => '',
			'after' => '',
		), array(), $this->tag);
	...
	public function get_related_image_size( $args = array() ){
		$image_size = $this->get_config('related_image_size');
		if( empty($image_size) ) {
			$image_size = $this->get_config('image_size');
		}
		$thumb_size = SLZ_Util::get_thumb_size( $image_size, $args );
		return $thumb_size;
	}
	public function get_related_post_setting( &$args = array() ){
		$columns = absint(slz_get_db_settings_option('blog-article/show/column', '3' ));
		if(empty($columns)){
			$columns = 3;
		}
		$args['related_columns'] = $columns;
		$display_info = array();
		if( $display = slz_get_db_settings_option('blog-article/show/display-info', array('image', 'info') ) ) {
			foreach( $display as $sel ){
				$display_info[$sel] = '';
			}
		}
		$args['related_display'] = $display_info;
	}
}
