...
class SLZ_Option_Type_Form_Builder_Item_Select extends SLZ_Option_Type_Form_Builder_Item {
	...
	public function get_thumbnails() {
		return array(
			array(
				'html' =>
					'<div class="item-type-icon-title" data-hover-tip="' . __( 'Add a Dropdown', 'slz' ) . '">' .
					'<div class="item-type-icon">' .
					'<img src="' . esc_attr( $this->get_uri( '/static/images/icon.png' ) ) . '" />' .
					'</div>' .
					'<div class="item-type-title">' . __( 'Dropdown', 'slz' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	protected function get_fixed_attributes( $attributes ) {
		...
		if ( empty( $attributes['options']['choices'] ) ) {
			$attributes['options']['choices'][] = __( 'Dropdown', 'slz' );
		}
	...
	public function frontend_validate( array $item, $input_value ) {
		...
		$messages = array(
			...
			'not_existing_choice' => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( '{label}: Submitted data contains not existing choice', 'slz' )
			),
		);
...
SLZ_Option_Type_Builder::register_item_type( 'SLZ_Option_Type_Form_Builder_Item_Select' );
