...
class SLZ_Option_Type_Form_Builder_Item_Email extends SLZ_Option_Type_Form_Builder_Item {
	...
	public function get_thumbnails() {
		return array(
			array(
				'html' =>
					'<div class="item-type-icon-title" data-hover-tip="' . __( 'Add an Email field', 'slz' ) . '">' .
					'<div class="item-type-icon">' .
					'<img src="' . esc_attr( $this->get_uri( '/static/images/icon.png' ) ) . '" />' .
					'</div>' .
					'<div class="item-type-title">' . __( 'Email', 'slz' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	public function frontend_validate( array $item, $input_value ) {
		...
		$messages = array(
			...
			'incorrect' => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must contain a valid email', 'slz' )
			),
		);
...
SLZ_Option_Type_Builder::register_item_type( 'SLZ_Option_Type_Form_Builder_Item_Email' );
