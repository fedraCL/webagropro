...
class SLZ_Option_Type_Form_Builder_Item_Text extends SLZ_Option_Type_Form_Builder_Item {
	...
	public function get_thumbnails() {
		return array(
			array(
				'html' =>
					'<div class="item-type-icon-title" data-hover-tip="' . __( 'Add a Single Line Text', 'slz' ) . '">' .
					'<div class="item-type-icon"><img src="' . esc_attr( $this->get_uri( '/static/images/icon.png' ) ) . '" /></div>' .
					'<div class="item-type-title">' . __( 'Single Line Text', 'slz' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	public function enqueue_static() {
		...
		wp_localize_script(
			...
			array(
				'l10n'     => array(
					'item_title'      => __( 'Single Line Text', 'slz' ),
					'label'           => __( 'Label', 'slz' ),
...
SLZ_Option_Type_Builder::register_item_type( 'SLZ_Option_Type_Form_Builder_Item_Text' );
