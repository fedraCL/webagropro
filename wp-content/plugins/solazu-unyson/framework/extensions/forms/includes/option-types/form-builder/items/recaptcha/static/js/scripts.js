	...
	var ItemView = builder.classes.ItemView.extend({
			...
			'<div class="slz-form-builder-item-style-default slz-form-builder-item-type-'+ currentItemType +'">'+
				...
				'<div class="slz-form-item-preview">'+
					'<div class="slz-form-item-preview-label">'+
						'<div class="slz-form-item-preview-label-wrapper"><label data-hover-tip="<%- edit_label %>"><%- label %></label> </div>'+
						'<div class="slz-form-item-preview-label-edit"><!-- --></div>'+
					'</div>'+
					'<div class="slz-row slz-form-item-recaptcha-preview">'+
                        '<div class="alpha slz-col-xs-6">'+
                            '<div class="slz-form-item-preview-input"><input type="text" placeholder="<%- site_key %>" value="<%- default_value %>"></div>'+
                        '</div>'+
                        '<div class="omega slz-col-xs-6">'+
                            '<div class="slz-form-item-preview-input"><input type="text" placeholder="<%- secret_key %>" value="<%- default_value %>"></div>'+
                        '</div>'+
					'</div>'+
				'</div>'+
			'</div>'
		...
		render: function () {
			this.defaultRender({
				...
				site_key: slz.opg('recaptcha/site-key', this.model.get('options'))
                    ? slz.opg('recaptcha/site-key', this.model.get('options'))
                    : localized.l10n.site_key,
				secret_key: slz.opg('recaptcha/secret-key', this.model.get('options'))
                    ? slz.opg('recaptcha/secret-key', this.model.get('options'))
                    : localized.l10n.secret_key,
		...
		updateDefaultValueFromPreviewInput: function() {
			...
			values.default_value = this.$('.slz-form-item-preview-input input').val();
