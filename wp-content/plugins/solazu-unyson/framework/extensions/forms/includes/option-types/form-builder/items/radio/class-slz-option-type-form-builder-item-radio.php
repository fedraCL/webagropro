...
class SLZ_Option_Type_Form_Builder_Item_Radio extends SLZ_Option_Type_Form_Builder_Item {
		...
		return array(
			array(
				'html' =>
					...
					'</div>' .
					'<div class="item-type-title">' . __( 'Single Choice', 'slz' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	public function enqueue_static() {
		...
		wp_localize_script(
			...
			array(
				'l10n'     => array(
					'item_title'      => __( 'Single Choice', 'slz' ),
					'label'           => __( 'Label', 'slz' ),
	...
	protected function get_fixed_attributes( $attributes ) {
		...
		if ( empty( $attributes['options']['choices'] ) ) {
			$attributes['options']['choices'][] = __( 'Single Choice', 'slz' );
		}
...
SLZ_Option_Type_Builder::register_item_type( 'SLZ_Option_Type_Form_Builder_Item_Radio' );
