...
require dirname( __FILE__ ) . '/includes/extends/class-slz-extension-forms-form.php';

/**
 * Build frontend forms
 */
class SLZ_Extension_Forms extends SLZ_Extension {
	...
	protected function _init() {
		$this->frontend_form = new SLZ_Form( 'slz_form', array(
	...
	public function _frontend_form_render( $data ) {
		...
		$form_type_input_name = 'slz_ext_forms_form_type';
		$form_type_input_id   = 'slz_ext_forms_form_id';

		$data['attr']['data-slz-ext-forms-type'] = $form_type;
		$data['attr']['class'] = apply_filters('slz:ext:forms:attr:class', $data['attr']['class']);
		...
		/**
		 * @var SLZ_Ext_Forms_Type $form_type
		 */
		$form_type = slz_ext( $form_type );

		/**
		 * @var SLZ_Option_Type_Form_Builder $builder
		 */
		$builder = slz()->backend->option_type( $form_type->get_form_builder_type() );

		echo $builder->frontend_render( $data['data']['builder_value'], SLZ_Request::POST() );
	...
	public function _frontend_form_validate( $errors ) {
		...
		$form_type = SLZ_Request::POST( 'slz_ext_forms_form_type' );

		if ( empty( $form_id ) || empty( $form_type ) ) {
			return array(
				'invalid-form-id' => __( 'Unable to process the form', 'slz' )
			);
		}

		/**
		 * @var SLZ_Ext_Forms_Type $form_instance
		 */
		$form_instance = slz_ext( $form_type );
		 ...
		 * @var SLZ_Option_Type_Form_Builder $builder
		 */
		$builder = slz()->backend->option_type( $form_instance->get_form_builder_type() );

		if ( ! $builder instanceof SLZ_Option_Type_Form_Builder ) {
			return array(
				'invalid-form-id' => __( 'Unable to process the form', 'slz' )
			);
		}
	...
	/**
	 * @param array $slz_form_data
	 *
	 * @return array
	 *
	 * @internal
	 */
	public function _frontend_form_save( $slz_form_data ) {
		$form_id   = SLZ_Request::POST( 'slz_ext_forms_form_id' );
		$form_type = SLZ_Request::POST( 'slz_ext_forms_form_type' );
		 ...
		 */
		$slz_form_data['redirect'] = slz_current_url();

		if ( empty( $builder_value_json_array ) ) {
			return $slz_form_data;
		}
		...
		if ( is_array( $process_data ) ) {
			if ( isset( $process_data['redirect'] ) ) {
				$slz_form_data['redirect'] = $process_data['redirect'];
			}
		}

		do_action('slz_ext_forms_frontend_submit', array(
			'id' => $form_id,
			'type' => $form_type,
	...
	public function _child_extension_is_valid( $child_extension_instance ) {
		return is_subclass_of( $child_extension_instance, 'SLZ_Extension_Forms_Form' );
	}
