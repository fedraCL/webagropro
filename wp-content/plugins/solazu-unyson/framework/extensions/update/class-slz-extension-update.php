...
require dirname(__FILE__) .'/includes/extends/class-slz-ext-update-service.php';

class SLZ_Extension_Update extends SLZ_Extension
{
	/**
	 * {@inheritdoc}
	 */
	...
	{
		return is_subclass_of($child_extension_instance, 'SLZ_Ext_Update_Service');
	}
	...
	private function add_actions()
	{
		...
		add_action('update-core-custom_'. 'slz-update-framework',  array($this, '_action_update_framework'));
		add_action('update-core-custom_'. 'slz-update-theme',      array($this, '_action_update_theme'));
		add_action('update-core-custom_'. 'slz-update-extensions', array($this, '_action_update_extensions'));
	...
	{
		return SLZ_WP_Filesystem::real_path_to_filesystem_path(
			apply_filters('slz_tmp_dir', slz_fix_path(WP_CONTENT_DIR) .'/tmp')
		);
	}
	...
	{
		$cache_key = 'slz_ext_update/updates';
		...
		try {
			...
			SLZ_Cache::set($cache_key, $updates);
	...
	private function get_extensions_with_updates($force_check = false)
	{
		...
		$theme_ext_requirements = slz()->theme->manifest->get('requirements/extensions');

		foreach (slz()->extensions->get_all() as $ext_name => $extension) {
			...
			foreach ($services as $service_name => $service) {
				/** @var $service SLZ_Ext_Update_Service */
	...
	private function get_framework_update($force_check = false)
	{
		...
		foreach ($this->get_children() as $service_name => $service) {
			...
			if (!version_compare($fixed_latest_version, slz()->manifest->get_version(), '>')) {
				// we already have latest version
				continue;
			}
	...
	private function get_theme_update($force_check = false)
	{
		...
		foreach ($this->get_children() as $service_name => $service) {
			...
			if (!version_compare($fixed_latest_version, slz()->theme->manifest->get_version(), '>')) {
				// we already have latest version
				continue;
			}
	...
	private function maintenance_mode($enable = false)
	{
		...
		if ($wp_filesystem->exists($file_path)) {
			if (!$wp_filesystem->delete($file_path)) {
				trigger_error(__('Cannot delete: ', 'slz') . $file_path, E_USER_WARNING);
			}
		}
		...
		if ($enable) {
			...
			if (!$wp_filesystem->put_contents($file_path, '<?php $upgrading = ' . time() . '; ?>', FS_CHMOD_FILE)) {
				trigger_error(__('Cannot create: ', 'slz') . $file_path, E_USER_WARNING);
			}
		}
	}
	...
	private function update($data, $merge_extensions = false)
	{
		...
		do {
			...
			// create temporary directory
			{
				if ($wp_filesystem->exists($tmp_download_dir)) {
					...
					if (!$wp_filesystem->rmdir($tmp_download_dir, true)) {
						$error = __('Cannot remove old temporary directory: ', 'slz') . $tmp_download_dir;
						break;
					}
				}

				if (!SLZ_WP_Filesystem::mkdir_recursive($tmp_download_dir)) {
					$error = __('Cannot create directory: ', 'slz') . $tmp_download_dir;
					break;
				}
			}

			$skin->feedback(sprintf(__('Downloading the %s...', 'slz'), $title));
			{
				$downloaded_dir = call_user_func_array($download_callback, $download_callback_args);

				if (!$downloaded_dir) {
					$error = sprintf(__('Cannot download the %s.', 'slz'), $title);
					break;
			...
			$skin->feedback(sprintf(__('Installing the %s...', 'slz'), $title));
			{
				// remove all files from destination directory
				{
					$dir_files = $wp_filesystem->dirlist($wp_fs_destination_dir, true);
					if ($dir_files === false) {
						$error =__('Cannot access directory: ', 'slz') . $wp_fs_destination_dir;
						break;
					}
					...
					foreach ($dir_files as $file) {
						...
						if (!$wp_filesystem->delete($file_path, true, $file['type'])) {
							$error = __('Cannot remove: ', 'slz') . $file_path;
							break 2;
						}
					}
				}
				...
				// move all files from the temporary directory to the destination directory
				{
					...
					if ($dir_files === false) {
						$error = __('Cannot access directory: ', 'slz') . $downloaded_dir;
						break;
					}
			...
			$skin->feedback(sprintf(__('The %s has been successfully updated.', 'slz'), $title));
		} while(false);
		...
		if ($wp_filesystem->exists($tmp_download_dir)) {
			if ( ! $wp_filesystem->delete( $tmp_download_dir, true, 'd' ) ) {
				$error = sprintf( __( 'Cannot remove temporary directory "%s".', 'slz' ), $tmp_download_dir );
			}
		}

		if ($error) {
			if (!is_wp_error($error)) {
				$error = new WP_Error( 'slz_ext_update_failed', (string)$error );
			}
	...
	private function merge_extensions($source_dir, $destination_dir)
	{
		...
		$wp_error_id = 'slz_ext_update_merge_extensions';
		...
		if (!$wp_filesystem->exists($destination_dir)) {
			...
			if (!$wp_filesystem->move($source_dir, $destination_dir)) {
				return new WP_Error($wp_error_id,
					sprintf(__('Cannot move "%s" to "%s"', 'slz'), $source_dir, $destination_dir)
				);
			}
			return true;
		}
		...
		if ($source_ext_dirs === false) {
			return new WP_Error($wp_error_id,
				__('Cannot access directory: ', 'slz') . $source_dir
			);
		}
		...
		foreach ($source_ext_dirs as $ext_dir) {
...

			{
				...
				if ($source_ext_files === false) {
					return new WP_Error($wp_error_id,
						__('Cannot access directory: ', 'slz') . $source_extension_dir
					);
				}
			...
			// prepare destination
			{
				...
				if (!$wp_filesystem->exists($destination_extension_dir)) {
					if (!SLZ_WP_Filesystem::mkdir_recursive($destination_extension_dir)) {
						return new WP_Error($wp_error_id,
							__('Cannot create directory: ', 'slz') . $destination_extension_dir
						);
					}
				}
				...
				// remove everything except the extensions/ dir
				{
					...
					if ($dest_ext_files === false) {
						return new WP_Error($wp_error_id,
							__('Cannot access directory: ', 'slz') . $destination_extension_dir
						);
					}
					...
					foreach ($dest_ext_files as $dest_ext_file) {
						...
						if (!$wp_filesystem->delete($dest_ext_file_path, true, $dest_ext_file['type'])) {
							return new WP_Error($wp_error_id,
								__('Cannot delete: ', 'slz') . $dest_ext_file_path
							);
						}
					}
				}
			...
			// move files from source to destination extension directory
			{
				...
				foreach ($source_ext_files as $source_ext_file) {
					...
					if (!$wp_filesystem->move($source_ext_file_path, $dest_ext_file_path)) {
						return new WP_Error($wp_error_id,
							sprintf(__('Cannot move "%s" to "%s"', 'slz'),
	...
	{
		$nonce_name = '_nonce_slz_ext_update_framework';
		if (!isset($_POST[$nonce_name]) || !wp_verify_nonce($_POST[$nonce_name])) {
			wp_die(__('Invalid nonce.', 'slz'));
		}

		{
			if (!class_exists('_SLZ_Ext_Update_Framework_Upgrader_Skin')) {
				slz_include_file_isolated(
					$this->get_declared_path('/includes/classes/class--slz-ext-update-framework-upgrader-skin.php')
				);
			}

			$skin = new _SLZ_Ext_Update_Framework_Upgrader_Skin(array(
				'title' => __('Framework Update', 'slz'),
			));
		}
		...
		do {
			if (!SLZ_WP_Filesystem::request_access(slz_get_framework_directory(), slz_current_url(), array($nonce_name))) {
				break;
			}
			...
			if ($update === false) {
				$skin->error(__('Failed to get framework latest version.', 'slz'));
				break;
			...
			/** @var SLZ_Ext_Update_Service $service */
			$service = $this->get_child($update['service']);

			$update_result = $this->update(array(
				'wp_fs_destination_dir' => SLZ_WP_Filesystem::real_path_to_filesystem_path(
					slz_get_framework_directory()
				),
	...
	{
		$nonce_name = '_nonce_slz_ext_update_theme';
		if (!isset($_POST[$nonce_name]) || !wp_verify_nonce($_POST[$nonce_name])) {
			wp_die(__('Invalid nonce.', 'slz'));
		}

		{
			if (!class_exists('_SLZ_Ext_Update_Theme_Upgrader_Skin')) {
				slz_include_file_isolated(
					$this->get_declared_path('/includes/classes/class--slz-ext-update-theme-upgrader-skin.php')
				);
			}

			$skin = new _SLZ_Ext_Update_Theme_Upgrader_Skin(array(
				'title' => __('Theme Update', 'slz'),
			));
		}
		...
		do {
			if (!SLZ_WP_Filesystem::request_access(get_template_directory(), slz_current_url(), array($nonce_name))) {
				break;
			}
			...
			if ($update === false) {
				$skin->error(__('Failed to get theme latest version.', 'slz'));
				break;
			} elseif (is_wp_error($update)) {
				$skin->error($update);
				break;
	...
	{
		$nonce_name = '_nonce_slz_ext_update_extensions';
		if (!isset($_POST[$nonce_name]) || !wp_verify_nonce($_POST[$nonce_name])) {
			wp_die(__('Invalid nonce.', 'slz'));
		}

		$form_input_name = 'extensions';
		$extensions_list = SLZ_Request::POST($form_input_name);

		if (empty($extensions_list)) {
			SLZ_Flash_Messages::add(
				'slz_ext_update',
				__('Please check the extensions you want to update.', 'slz'),
				'warning'
			);
		...
		{
			if (!class_exists('_SLZ_Ext_Update_Extensions_Upgrader_Skin')) {
				slz_include_file_isolated(
					$this->get_declared_path('/includes/classes/class--slz-ext-update-extensions-upgrader-skin.php')
				);
			}

			$skin = new _SLZ_Ext_Update_Extensions_Upgrader_Skin(array(
				'title' => __('Extensions Update', 'slz'),
			));
		}
		...
		do {
			...
			if (empty($updates)) {
				$skin->error(__('No extensions updates found.', 'slz'));
				break;
			}

			foreach ($extensions_list as $extension_name) {
				if (!($extension = slz()->extensions->get($extension_name))) {
					$skin->error(
						sprintf(__('Extension "%s" does not exist or is disabled.', 'slz'), $extension_name)
					);
					continue;
				}
				...
				if (!isset($updates[$extension_name])) {
					$skin->error(
						sprintf(__('No update found for the "%s" extension.', 'slz'), $extension->manifest->get_name())
					);
					continue;
				}
				...
				$update_result = $this->update(array(
					...
					'skin' => $skin,
					'title' => sprintf(__('%s extension', 'slz'), $extension->manifest->get_name()),
				), true);
	...
	public function _action_admin_notices() {
		if (
			get_current_screen()->parent_base === slz()->extensions->manager->get_page_slug()
			&&
		...
		echo '<div class="notice notice-warning"><p>'
			. sprintf(
				esc_html__('New extensions updates available. %s', 'slz'),
				slz_html_tag('a', array(
					'href' => self_admin_url('update-core.php') .'#slz-ext-update-extensions',
				), esc_html__('Go to Updates page', 'slz'))
			)
			. '</p></div>';
	}
}
