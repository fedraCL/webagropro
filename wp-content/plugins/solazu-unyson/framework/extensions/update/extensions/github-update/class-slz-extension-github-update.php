 ...
 */
class SLZ_Extension_Github_Update extends SLZ_Ext_Update_Service
{
	/**
	...
	{
		return apply_filters('slz_github_api_url', 'https://api.github.com') . $append;
	}
	...
	private function fetch_latest_version($user_slash_repo)
	{
		...
		if (($response_code = intval(wp_remote_retrieve_response_code($response))) !== 200) {
			if ($response_code === 403) {
				...
				if ($json_response) {
					return new WP_Error(
						'slz_ext_update_github_fetch_releases_failed',
						__('Github error:', 'slz') .' '. $json_response['message']
					);
				}
			}
			...
			if ($response_code) {
				return new WP_Error(
					'slz_ext_update_github_fetch_releases_failed',
					sprintf(
						__( 'Failed to access Github repository "%s" releases. (Response code: %d)', 'slz' ),
				...
				return new WP_Error(
					'slz_ext_update_github_fetch_releases_failed',
					sprintf(
						__( 'Failed to access Github repository "%s" releases.', 'slz' ),
						$user_slash_repo
					)
				);
			}
		...
		if (empty($release)) {
			return new WP_Error(
				'slz_ext_update_github_fetch_no_releases',
				sprintf(__('No releases found in repository "%s".', 'slz'), $user_slash_repo)
			);
		}
	...
	private function get_latest_version($user_slash_repo, $force_check, $title)
	{
		if (!preg_match($this->manifest_key_regex, $user_slash_repo)) {
			return new WP_Error('slz_ext_update_github_manifest_invalid',
				sprintf(
					__('%s manifest has invalid "github_update" parameter. Please use "user/repo" format.', 'slz'),
					$title
				)
			);
		}

		$transient_id = 'slz_ext_upd_gh_slz'; // the length must be 45 characters or less
		...
		if (empty($latest_version)) {
			return new WP_Error(
				'slz_ext_update_github_failed_fetch_latest_version',
				sprintf(
					__('Failed to fetch %s latest version from github "%s".', 'slz'),
	...
	private function download($user_slash_repo, $version, $wp_filesystem_download_directory, $title)
	{
		...
		if ($response_code !== 200) {
			if ($response_code === 403) {
				...
				if ($json_response) {
					return new WP_Error(
						'slz_ext_update_github_download_releases_failed',
						__('Github error:', 'slz') .' '. $json_response['message']
					);
				}
			}
		...
		if (empty($release)) {
			return new WP_Error(
				'slz_ext_update_github_download_no_release',
				sprintf(
					__('%s github repository "%s" does not have the "%s" release.', 'slz'),
		...
		if (intval(wp_remote_retrieve_response_code($response)) !== 200) {
			return new WP_Error(
				'slz_ext_update_github_download_failed',
				sprintf(__('Cannot download %s zip.', 'slz'), $title)
			);
		}
		...
		if (!$wp_filesystem->put_contents($zip_path, $response['body'])) {
			return new WP_Error(
				'slz_ext_update_github_save_download_failed',
				sprintf(__('Cannot save %s zip.', 'slz'), $title)
			);
		}
		...
		if (!$wp_filesystem->delete($zip_path, false, 'f')) {
			return new WP_Error(
				'slz_ext_update_github_remove_downloaded_zip_failed',
				sprintf(__('Cannot remove %s zip.', 'slz'), $title)
			);
		}
		...
		if (!$unzipped_dir_files) {
			return new WP_Error(
				'slz_ext_update_github_unzipped_dir_fail',
		...
		return new WP_Error(
			'slz_ext_update_github_unzipped_dir_not_found',
			sprintf(__('The unzipped %s directory not found.', 'slz'), $title)
		);
	}
	...
	{
		$user_slash_repo = slz()->manifest->get($this->manifest_key);
	...
	public function _download_framework($version, $wp_filesystem_download_directory)
	{
		return $this->download(
			slz()->manifest->get($this->manifest_key),
			$version,
			$wp_filesystem_download_directory,
			__('Framework', 'slz')
		);
	...
	{
		$user_slash_repo = slz()->theme->manifest->get($this->manifest_key);
	...
	{
		return $this->download(
			slz()->theme->manifest->get($this->manifest_key),
			$version,
			$wp_filesystem_download_directory,
			__('Theme', 'slz')
		);
	 ...
	 * @internal
	 */
	public function _get_extension_latest_version(SLZ_Extension $extension, $force_check)
	{
		...
		return $this->get_latest_version(
			$user_slash_repo,
			$force_check,
			sprintf(__('%s extension', 'slz'), $extension->manifest->get_name())
		);
	}
	 ...
	 * @internal
	 */
	public function _download_extension(SLZ_Extension $extension, $version, $wp_filesystem_download_directory)
	{
		return $this->download(
