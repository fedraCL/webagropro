<?php
$post_count = 1;
echo '<div class="slz-block-grid style-6 '. esc_attr( $class_position ) .'">';
	if( !empty( $block->query->posts ) ){
		foreach ($block->query->posts as $post) {
			...
			if( $post_count == 1 || $post_count == 2 ) {
				echo slz_render_view( $instance->locate_path( '/views/large_module.php' ), compact('module', 'post_count', 'block'));
			}else{
