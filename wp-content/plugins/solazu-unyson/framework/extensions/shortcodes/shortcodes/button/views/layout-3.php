...
//custom css
	...
	if ( !empty( $item['icon_bg_color'] ) ) {
	    $css = '.%1$s.slz-btn .btn-icon{background-color: %2$s;}';
	    $custom_css .= sprintf( $css, esc_attr( $item['button_class'] ), esc_attr($item['icon_bg_color']) );
	}
	if ( !empty( $item['icon_bg_hv_color'] ) ) {
	    $css = '.%1$s.slz-btn:hover .btn-icon{background-color: %2$s;}';
	    $custom_css .= sprintf( $css, esc_attr( $item['button_class'] ), esc_attr($item['icon_bg_hv_color']) );
	}
...
//out put
	$out = '<a href="'.esc_url($href).'" '.esc_attr($link_arr).' class="'.esc_attr( $item['button_class'] ).' slz-btn btn-absolute '.esc_attr($extra_class).'">';
