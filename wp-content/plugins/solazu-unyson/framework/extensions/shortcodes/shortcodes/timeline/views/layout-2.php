...
if ( !empty( $data['timeline_info'] ) ) {
	...
	echo '<div class="slz-timeline layout-2">';
		...
		foreach ( $items as $item ) {
			...
			if ( !empty( $item['timeline'] ) ) {
				if ( !empty( $link_arr['url'] ) ) {
					echo '<a class="milestone-time" href="'. esc_url( $link_arr['url'] ) .'" '. esc_attr( $link_arr['other_atts'] ) .'>'. esc_html( $item['timeline'] ) .'</a>';
				} else {
					echo '<div class="milestone-number">'. esc_html( $item['timeline'] ) .'</div>';
				}
			}
			if ( !empty( $item['title'] ) ) {
				echo '<div class="milestone-title">';
				if ( empty( $link_arr['url'] ) ) {
