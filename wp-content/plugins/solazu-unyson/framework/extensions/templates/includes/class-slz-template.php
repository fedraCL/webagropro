...
class SLZ_Template
{
	...
	/**
	 * Gets the path at which the template is located
	...
	/**
	 * Gets the uri at which the template is located
	...
	final public function locate_path($rel_path = '')
	{
		if (class_exists('SLZ_File_Cache')) {
			try {
				return SLZ_File_Cache::get($cache_key = 'ext:shcd:template-' . $this->tag .':path:'. $rel_path);
	...
	final public function locate_URI($rel_path = '')
	{
		if (class_exists('SLZ_File_Cache')) {
			try {
				return SLZ_File_Cache::get( $cache_key = 'ext:shcd:template-' . $this->tag . ':uri:' . $rel_path );
			} catch ( SLZ_File_Cache_Not_Found_Exception $e ) {
				$result = false;
	...
	public function get_options()
	{
		...
		return apply_filters('slz_template_get_options', $this->options, $this->tag);
	}
	 ...
	 * @return string
	 */
	final public function render( $module, $echo = true ,$args = array())
	{
		$view_file = $this->locate_path('/views/view.php');
		if (!$view_file) {
			trigger_error(
				sprintf(__('No default view (views/view.php) found for template: %s', 'slz'), $this->tag),
				E_USER_ERROR
			);
		}
		...
		$view_extra = apply_filters('slz_template_render_view', array(
			'before' => '',
			'after' => '',
		), array(), $this->tag);

		$result = $view_extra['before'] .
				slz_render_view($view_file, compact( 'module','args' )) .
				$view_extra['after'];
		if( $echo ) {
			echo $result;
		} else {
