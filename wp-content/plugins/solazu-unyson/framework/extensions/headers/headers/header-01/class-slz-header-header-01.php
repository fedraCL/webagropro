...
class SLZ_Header_Header_01 extends SLZ_Header {
	...
	public function render( $echo = true )
	{
		...
		$options = slz_get_db_settings_option('slz-header-style-group/header_01', '');
		...
		if ( !empty ( $options ) ) {
			...
			if ( !empty ( $topbar ) && $topbar['selected-value'] == 'yes' ){
				...
				if ( !empty ( $topbar_style ) ){
                    ...
                    $topbar_bg_color = slz_get_db_post_option( get_the_ID(), 'header-top-bg-color' );
                    if($topbar_bg_color['id'] == 'slz-custom' && !empty($topbar_bg_color['color']) ) {
                        $bg_color = $topbar_bg_color['color'];
                    }
					...
					if ( !empty ( $border_color ) ) {
						...
						$custom_css .= '.slz-header-topbar { border-bottom-color: ' . esc_attr( $border_color ) . ' 
						!important}';
					...
					if ( !empty ( $topbar_style['social-icon-size'] ) && is_numeric( $topbar_style['social-icon-size'] ) ) {
						...
						$custom_css .= '.slz-header-topbar .social a i { font-size: ' . esc_attr( $topbar_style['social-icon-size'] ) . 'px }';
