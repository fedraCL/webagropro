...
class SLZ_Top_Post
{
	...
	/**
	 * Gets the path at which thetop post is located
	...
	/**
	 * Gets the uri at which thetop post is located
	...
	public function get_options()
	{
		...
		return apply_filters('slz_top_post_get_options', $this->options, $this->tag);
	}
	 ...
	 * @return string
	 */
	final public function render( $posts, $echo = true )
	{
		...
		$view_extra = apply_filters('slz_top_post_render_view', array(
			'before' => '',
			'after' => '',
		), array(), $this->tag);
		$attr = $this->get_data();
		...
		$result = $view_extra['before'] .
				slz_render_view($view_file, compact( 'attr' , 'posts' )) .
				$view_extra['after'];
		if( $echo ) {
			echo $result;
		} else {
