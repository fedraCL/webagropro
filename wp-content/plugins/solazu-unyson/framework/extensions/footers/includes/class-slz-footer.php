...
class SLZ_Footer
{
	...
	/**
	 * Gets the path at which the footer is located
	...
	/**
	 * Gets the uri at which the footer is located
	...
	final public function locate_path($rel_path = '')
	{
		if (class_exists('SLZ_File_Cache')) {
			try {
				return SLZ_File_Cache::get($cache_key = 'ext:shcd:footer-' . $this->tag .':path:'. $rel_path);
	...
	final public function locate_URI($rel_path = '')
	{
		if (class_exists('SLZ_File_Cache')) {
			try {
				return SLZ_File_Cache::get( $cache_key = 'ext:shcd:footer-' . $this->tag . ':uri:' . $rel_path );
			} catch ( SLZ_File_Cache_Not_Found_Exception $e ) {
				$result = false;
	...
	public function get_options()
	{
		...
		return apply_filters('slz_footer_get_options', $this->options, $this->tag);
	}
	...
	public function render( $echo = true )
	{
		...
		if (!$view_file) {
			trigger_error(
				sprintf(__('No default view (views/view.php) found for footer: %s', 'slz'), $this->tag),
				E_USER_ERROR
			);
		}
		...
		$view_extra = apply_filters('slz_footer_render_view', array(
			'before' => '',
			'after' => '',
		), array(), $this->tag);
