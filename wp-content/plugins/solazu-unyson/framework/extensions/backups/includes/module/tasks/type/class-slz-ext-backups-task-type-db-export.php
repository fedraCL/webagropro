 ...
 * }
 */
class SLZ_Ext_Backups_Task_Type_DB_Export extends SLZ_Ext_Backups_Task_Type {
	public function get_type() {
		return 'db-export';
	}

	public function get_title(array $args = array(), array $state = array()) {
		return __('Database export', 'slz');
	}
	...
	public function execute(array $args, array $state = array()) {
		...
		if (!isset($tables[$state['table']])) {
			return new WP_Error(
				'table_disappeared', __('Database table disappeared', 'slz')
			);
		}
		...
		while (time() - $started_time < $timeout) {
			// open file for writing
			{
				...
				if (!file_exists($file_path)) {
					if (!($fp = @fopen($file_path, 'w'))) {
						return new WP_Error(
							'cannot_create_file', __('Cannot create file', 'slz') .': '. $file_path
						);
					}
				} else {
					if (!($fp = @fopen($file_path, 'a'))) {
						return new WP_Error(
							'cannot_reopen_file', __('Cannot reopen file', 'slz') .': '. $file_path
						);
					}
				}
			}
			...
			if ($state['limit'] == 0) { // create table before data insert
				...
				if (empty($sql)) {
					fclose($fp);
					return new WP_Error(
						'create_table_sql', __('Cannot export CREATE TABLE sql', 'slz')
					);
				} else {
					$sql = $sql[0];
				}
			...
			if (!($column = $this->get_table_order_by_column($state['table']))) {
				...
				if (is_null($state['table'])) {
					...
					fclose($fp);
					return new WP_Error(
						'no_next_table', __('Cannot get next database table', 'slz')
					);
				} else {
					$state['limit'] = 0;
				}
			...
			foreach ($wpdb->get_results(
				...
				if ('options' === $state['table']) {
					if (
						...
						||
						apply_filters('slz_ext_backups_db_export_exclude_option', false,
