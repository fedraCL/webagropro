...
class _SLZ_Ext_Backups_Module_Tasks extends _SLZ_Ext_Backups_Module {
	/**
	 * @var string
	 *
	 ...
	 * ]
	 */
	private static $wp_option_active_task_collection = 'slz:ext:backups:active_task_collection';
	 ...
	 * ]
	 */
	private static $wp_option_pending_task_collections = 'slz:ext:backups:pending_task_collections';

	private static $wp_ajax_action = 'slz:ext:backups:continue-pending-task';

	private static $skip_shutdown_function = false;

	public function _init() {
		require_once dirname(__FILE__) .'/entity/class-slz-ext-backups-task.php';
		require_once dirname(__FILE__) .'/entity/class-slz-ext-backups-task-collection.php';
		...
		add_filter(
			...
			array($this, '_filter_slz_ext_backups_db_exclude_option'),
			10, 2
		);
		add_filter(
			'slz_ext_backups_db_restore_exclude_option',
			array($this, '_filter_slz_ext_backups_db_exclude_option'),
			10, 2
		);
		add_filter(
	...
	public function _get_task_types(SLZ_Access_Key $access_key) {
		if (!in_array($access_key->get_key(), array('slz:ext:backups', 'slz:ext:backups:tasks'))) {
			trigger_error('Method call denied', E_USER_ERROR);
		}

		if (is_null(self::$task_types)) {
			if (!class_exists('_SLZ_Ext_Backups_Task_Type_Register')) {
				require_once dirname(__FILE__) .'/class--slz-ext-backups-task-register.php';
			}

			$task_types = new _SLZ_Ext_Backups_Task_Type_Register();

			if (!class_exists('SLZ_Ext_Backups_Task_Type')) {
				require_once dirname(__FILE__) .'/class-slz-ext-backups-task-type.php';
			}

			do_action('slz_ext_backups_task_types_register', $task_types);
	 ...
	 * @param SLZ_Access_Key $access_key
	 *
	 * @return SLZ_Ext_Backups_Task_Type|null
	 * @internal
	 */
	public function _get_task_type($type, SLZ_Access_Key $access_key) {
		if (!in_array($access_key->get_key(), array('slz:ext:backups', 'slz:ext:backups:tasks'))) {
			trigger_error('Method call denied', E_USER_ERROR);
		}
	...
	private static function get_access_key() {
		if (empty(self::$access_key)) {
			self::$access_key = new SLZ_Access_Key('slz:ext:backups:tasks');
		}

		return self::$access_key;
	}

	private function do_task_fail_action(SLZ_Ext_Backups_Task $task) {
		if ($task->result_is_fail()) {
			do_action( 'slz:ext:backups:task:fail', $task );
		}
	}

	/**
	 * @param SLZ_Ext_Backups_Task_Collection|null $collection
	 *
	 * @return bool
	 *
	...
	private function set_active_task_collection($collection) {
		if ($collection instanceof SLZ_Ext_Backups_Task_Collection) {
	...
	/**
	 * @return SLZ_Ext_Backups_Task_Collection|null
	 */
	public function get_active_task_collection() {
		if ($db_value = get_option(
			...
			return $this->check_and_fix_tasks(
				SLZ_Ext_Backups_Task_Collection::from_array( $db_value )
			);
		} else {
			return $db_value;
		}
	...
	/**
	 * @param SLZ_Ext_Backups_Task_Collection[] $collections
	 *
	 * @return bool
	 */
	...
	public function get_pending_task_collections() {
		...
		while ($collection = array_shift($db_value)) {
			$collections[] = SLZ_Ext_Backups_Task_Collection::from_array($collection);
		}
	...
	private function check_and_fix_tasks($collection) {
		if ($collection instanceof SLZ_Ext_Backups_Task_Collection) {} else {
			return null;
		}
		...
		foreach ($collection->get_tasks() as $task) {
			...
			if ( ! $task->result_is_finished() ) {
				if ( ! $this->_get_task_type( $task->get_type(), self::get_access_key() ) ) {
					$task->set_result( new WP_Error(
						'type_not_registered', __( 'Task type not registered', 'slz' )
					));
				}
				...
				if ( $task->get_last_execution_end_time() ) { // step finished and should continue
					if ( $task->get_last_execution_end_time() + self::backups()->get_timeout() + 5 < time() ) {
						$task->set_result(new WP_Error(
							'execution_stopped', __( 'Execution stopped (next step did not started)', 'slz' )
						));
					}
				} else { // is currently executing
					{
						...
						if ( $task->get_last_execution_start_time() + $timeout + 1 < time() ) {
							$task->set_result( new WP_Error(
								'timeout', __( 'The execution failed. Please check error.log', 'slz' )
							) );
						}
					}
				}
			...
			if ( $task->result_is_finished() ) {
				if ( $task->result_is_fail() ) {
					do_action( 'slz:ext:backups:tasks:fail:id:'. $collection->get_id(), $collection );
					do_action( 'slz:ext:backups:tasks:fail', $collection );
					break;
				}
		...
		if ($finished) {
			...
			if (isset($task) && $task->result_is_fail()) {
				...
				do_action('slz:ext:backups:tasks:success:id:'. $collection->get_id(), $collection);
				do_action('slz:ext:backups:tasks:success', $collection);
			}

			do_action('slz:ext:backups:tasks:finish:id:'. $collection->get_id(), $collection);
			do_action('slz:ext:backups:tasks:finish', $collection);
	 ...
	 * @return null|SLZ_Ext_Backups_Task
	 */
					...
					if ($task->result_is_finished()) {
						{
							$task->set_last_execution_end_time(microtime(true));
							$task->set_result(new WP_Error(
								'invalid_execution_end_time', __('Invalid execution end time', 'slz')
							));
	 ...
	 * @return null|SLZ_Ext_Backups_Task with result null|array
	 */
	public function get_pending_task() {
		...
		if ($collection = $this->get_active_task_collection()) {
			 ...
			 * @param SLZ_Ext_Backups_Task_Collection $collection
			 * Note: $collection can be modified by reference http://stackoverflow.com/a/2715039
			 */
			do_action('slz:ext:backups:tasks:before_process', $collection);
	...
	private function execute_pending_task() {
		...
		if (!(
			...
			$task->set_result(new WP_Error(
				'invalid_execution_result', __('Invalid execution result', 'slz')
			));
		}
		...
		if ($task->result_is_finished()) {
			if ($task->result_is_fail()) {
				...
				do_action( 'slz:ext:backups:task:success', $task );
			}

			do_action( 'slz:ext:backups:task:executed:finished', $task );
		} else {
			do_action( 'slz:ext:backups:task:executed:unfinished', $task );
		}

		do_action( 'slz:ext:backups:task:executed', $task);
	 ...
	 * @internal
	 */
	public function _request_next_step_execution(SLZ_Access_Key $access_key) {
		if (!in_array($access_key->get_key(), array('slz:ext:backups', 'slz:ext:backups-demo'))) {
			trigger_error('Method call denied', E_USER_ERROR);
		}
	...
	public function _action_ajax_continue_pending_task() {
		if (
			...
			wp_send_json_error(new WP_Error('invalid_token', __('Invalid token', 'slz')));
		}
		...
		if (!isset($_POST['active_tasks_hash'])) {
			...
			wp_send_json_error(new WP_Error('invalid_tasks_hash', __('Invalid tasks hash', 'slz')));
		}
	 ...
	 * @param SLZ_Ext_Backups_Task_Collection $collection
	 */
	public function execute_task_collection(SLZ_Ext_Backups_Task_Collection $collection) {
	...
	private function get_dirs($full = false) {
		...
		$dirs = array(
			'uploads' => slz_fix_path($wp_upload_dir['basedir']),
			'plugins' => slz_fix_path(WP_PLUGIN_DIR),
			'themes'  => slz_fix_path(get_theme_root()),
		);

		if (is_multisite() && WPMU_PLUGIN_DIR) {
			$dirs['mu-plugins'] = slz_fix_path(WPMU_PLUGIN_DIR);
		}
	 ...
	 * @return SLZ_Ext_Backups_Task_Collection
	 */
	public function add_backup_tasks(SLZ_Ext_Backups_Task_Collection $collection, $full = false) {
		$full = (bool)$full;
		...
		$collection->add_task(new SLZ_Ext_Backups_Task(
			...
			array(
				...
				'exclude_paths' => ( is_multisite() && ($wp_upload_dir = wp_upload_dir()) )
					? array(slz_fix_path($wp_upload_dir['basedir']) .'/sites' => true)
					: array(),
			)
		));
		if (!$full) {
	...
	public function add_restore_tasks(
		SLZ_Ext_Backups_Task_Collection $collection,
		$full = false,
		$zip_path = null,
		...
		if (
			!$full
			&&
			apply_filters('slz:ext:backups:add-restore-task:image-sizes-restore', true, $collection)
		) {
			$collection->add_task(new SLZ_Ext_Backups_Task(
	...
	public function do_backup($full = false) {
		$this->execute_task_collection(
			$this->add_backup_tasks(
				new SLZ_Ext_Backups_Task_Collection(
	...
	public function do_restore($full = false, $zip_path, $filesystem_args = array()) {
		...
		$collection = new SLZ_Ext_Backups_Task_Collection(($full ? 'full' : 'content') .'-restore');
	 ...
	 * @return bool
	 */
	public function _filter_slz_ext_backups_db_exclude_option($exclude, $option_name) {
		if (
	 ...
	 * @return bool
	 */
	private function was_executed(SLZ_Ext_Backups_Task $task) {
	 ...
	 * @param SLZ_Ext_Backups_Task $task
	 */
	private function set_executed(SLZ_Ext_Backups_Task $task) {
		$path = $this->get_executed_tasks_path();
