...
class _SLZ_Ext_Backups_Module_Schedule extends _SLZ_Ext_Backups_Module {
	private static $wp_ajax_get_settings = 'slz:ext:backups:schedule:get-settings';
	private static $wp_ajax_set_settings = 'slz:ext:backups:schedule:set-settings';

	private static $wp_option_settings = 'slz:ext:backups:schedule:settings';

	private static $wp_cron_prefix = 'slz:ext:backups:cron:';
	...
	public function _init() {
		{
			add_action('slz:ext:backups:enqueue_scripts', array($this, '_action_enqueue_backup_scripts'));
		...
		{
			add_filter('slz:ext:backups:script_localized_data', array($this, '_filter_script_localized_data'));
			add_filter('slz_ext_backups_ajax_status_extra_response', array($this, '_filter_ajax_status_response_data'));

			add_filter('slz_ext_backups_db_export_exclude_option', array($this, '_filter_db_exclude_option'), 10, 2);
			add_filter('slz_ext_backups_db_restore_exclude_option', array($this, '_filter_db_exclude_option'), 10, 2);
			add_filter('slz_ext_backups_db_restore_keep_options', array($this, '_filter_db_restore_keep_options'));
	...
	private function get_options() {
		$options = slz_get_variables_from_file(
	...
	public function _filter_script_localized_data($data) {
		$data['schedule'] = array(
			...
			'popup_title' => __('Backup Schedule', 'slz'),
		);
	...
	public function _action_ajax_set_settings() {
		...
		foreach (array('full', 'content') as $type) {
			if ($type === 'full' && !slz_ext_backups_current_user_can_full()) {
	...
	public function _filter_cron_schedules($schedules) {
		$schedules['weekly'] = array(
			'interval' => 604800,
			'display' => __('Once Weekly', 'slz')
		);
		$schedules['monthly'] = array(
			'interval' => 2635200,
			'display' => __('Once a month', 'slz')
		);
	...
	public function _filter_ajax_status_response_data($data) {
		...
		foreach (array(
			'full' => esc_html__('Full', 'slz'),
			'content' => esc_html__('Content', 'slz')
		) as $type => $title) {
			$hook = self::$wp_cron_prefix . $type;
			...
			switch ($recurrence) {
				case 'monthly': $recurrence_title = esc_html__('Monthly', 'slz'); break;
				case 'weekly':  $recurrence_title = esc_html__('Weekly', 'slz'); break;
				case 'daily':   $recurrence_title = esc_html__('Daily', 'slz'); break;
			...
			$html[] =
				'<div class="updated"><p>'.
				/**/'<strong>'. $title .' '. esc_html__('Backup Schedule Active', 'slz') .'</strong>: '.
				/**/$recurrence_title .' | '. esc_html__('Next backup on', 'slz') .' '. $time .
				'</p></div>';
		}
		...
		if (empty($html)) {
			$html =
				'<div class="error"><p>' .
				/**/esc_html__( 'No backup schedule created yet! We advise you to do it asap!', 'slz' ) .
				'</p></div>';
		}
