...
class SLZ_Ext_Backups_Task_Type_Image_Sizes_Remove extends SLZ_Ext_Backups_Task_Type {
	public function get_type() {
		return 'image-sizes-remove';
	}

	public function get_title(array $args = array(), array $state = array()) {
		return __('Image Sizes Remove', 'slz');
	}
	...
	public function execute(array $args, array $state = array()) {
...

		{
			if (empty($args['uploads_dir'])) {
				return new WP_Error(
					'no_uploads_dir', __('Uploads dir not specified', 'slz')
				);
			} else {
				$args['uploads_dir'] = slz_fix_path($args['uploads_dir']);
