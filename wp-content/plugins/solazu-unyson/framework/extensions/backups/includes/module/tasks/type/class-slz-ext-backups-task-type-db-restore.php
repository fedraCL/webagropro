 ...
 */
class SLZ_Ext_Backups_Task_Type_DB_Restore extends SLZ_Ext_Backups_Task_Type {
	public function get_type() {
		return 'db-restore';
	}

	public function get_title(array $args = array(), array $state = array()) {
		return __( 'Database restore', 'slz' );
	}
	...
	public function get_custom_timeout(array $args, array $state = array()) {
		...
		return slz_ext('backups')->get_config('max_timeout'); // Fixed timeout error
	}
	...
	public function execute(array $args, array $state = array()) {
...

		{
			if (empty($state)) {
				if (!file_exists($args['dir'] .'/database.json.txt')) {
					if ($args['required']) {
						return new WP_Error(
							'no_db_file', __('Database file not found', 'slz')
						);
					} else {
						return true;
					}
		...
		if (!class_exists('SplFileObject')) {
			return new WP_Error(
				'no_spl_file_object',
				__('PHP class SplFileObject is required but not available. Please contact your hosting', 'slz')
			);
		}
		...
		if ($state['task'] === 'cleanup') {
			...
			return new WP_Error(
				'invalid_sub_task',
				sprintf(__( 'Invalid sub task %s', 'slz' ), $state['task'])
			);
		}
	...
	private function do_cleanup(array $args, array $state) {
		...
		if ($table_names = $wpdb->get_col($wpdb->prepare(
			...
			if (!$wpdb->query('DROP TABLE '. esc_sql(
				...
				return new WP_Error(
					'drop_tmp_table_fail',
					sprintf(__('Cannot drop temporary table: %s', 'slz'), $table_name)
	...
	private function do_inspect(array $args, array $state) {
...

		{
			try {
				...
				$fo = null;
				return new WP_Error(
					'cannot_open_file', __('Cannot open db file', 'slz')
				);
			}
			...
			try {
				...
				$fo = null;
				return new WP_Error(
					'cannot_move_file_cursor', __( 'Cannot move cursor in db file', 'slz' )
				);
			}
		}

		$started_time = time();
		$timeout      = slz_ext( 'backups' )->get_timeout() - $this->get_timeout_padding();
			...
			if ( $line = $fo->current() ) {
				if ( is_null( $line = json_decode( $line, true ) ) ) {
					...
					return new WP_Error(
						'line_decode_fail',
						sprintf(
							__( 'Failed to decode line %d from db file.', 'slz' ) .' '. slz_get_json_last_error_message(),
							$state['step'] + 1
						)
					);
				}
				...
				return new WP_Error(
					'line_read_fail',
					sprintf(__( 'Cannot read line %d from db file', 'slz' ), $state['step'] + 1)
				);
			} else {
				if (
					...
					return new WP_Error(
						'params_not_found', __( 'Required params not found', 'slz' )
					);
				}
				...
				// decide if it's full backup or not
				{
					...
					if (is_null($state['full'])) {
						...
						return new WP_Error(
							...
							__('Cannot do full db restore because backup is missing some tables', 'slz')
						);
					}
				}
	...
	private function do_import(array $args, array $state) {
		 ...
		 */
		{
			...
			// Prepare clean search->replace strings
			{
				...
				foreach ($search_replace as $search => $replace) {
					$search_replace[
						slz_get_url_without_scheme($search)
					] = slz_get_url_without_scheme($replace);
				}
			}
			...
			foreach ($search_replace as $search => $replace) {
				 ...
				 */
				if ($shortcodes_ext = slz_ext('shortcodes')) { /** @var SLZ_Extension_Shortcodes $shortcodes_ext */
		...
		if (
			...
			$replace_option_names = array_merge(
				/** @since 2.0.12 */
				apply_filters('slz_ext_backups_db_restore_option_names_replace', array(), array(
		...
		$server_requirements = slz()->theme->manifest->get('server_requirements');
		$sql_is_supported = version_compare($wpdb->db_version(), $server_requirements['server']['mysql_version'], '>');
		...
		while ( time() - $started_time < $timeout ) {
			if ( $line = $fo->current() ) {
				...
				switch ( $line['type'] ) {
					case 'table':
						...
						if ( false === $wpdb->query( 'DROP TABLE IF EXISTS ' . esc_sql( $tmp_table_name ) ) ) {
							...
							return new WP_Error(
								'tmp_table_drop_fail',
								sprintf( __( 'Failed to drop tmp table %s', 'slz' ), $tmp_table_name )
								.($wpdb->last_error ? '. '. $wpdb->last_error : '')
							);
						}

						{
...

							{
								...
								foreach ( $line['data']['columns'] as $col_name => $col_opts ) {
									$col_opt_check = $utf8mb4_is_supported
											? $col_opts
									...
									$cols_sql[] = '`'. esc_sql( $col_name ) .'` '. (
										$sql_is_supported
											? $col_opt_check
											: str_replace( 'unicode_520_ci', 'unicode_ci', $col_opt_check )
										);
								}
						...
						if ( false === $wpdb->query( $sql ) ) {
							...
							return new WP_Error(
								...
								sprintf( __( 'Failed to create tmp table %s', 'slz' ), $tmp_table_name )
								.($wpdb->last_error ? '. '. $wpdb->last_error : '')
							);
						}
					...
					case 'row':
						if ( ! isset( $state['tables'][ $line['data']['table'] ] ) ) {
							...
							return new WP_Error(
								'invalid_table',
								sprintf( __( 'Tried to insert data in table that was not imported %s', 'slz' ), $line['data']['table'] )
							);
							...
							&&
							apply_filters('slz_ext_backups_db_restore_exclude_option', false,
								...
								if ( false === $wpdb->query( $sql ) ) {
									if (
										...
										return new WP_Error(
											'insert_fail',
											sprintf(
												__('Failed to insert row from line %d into table %s', 'slz'),
					...
					default:
						...
						return new WP_Error(
							'invalid_json_type',
							sprintf( __( 'Invalid json type %s in db file', 'slz' ), $line['type'] )
						);
				}
			} elseif ( $line === false && ! $fo->eof() ) {
				$fo = null;
				...
				return new WP_Error(
					'line_read_fail', __( 'Cannot read line from db file', 'slz' )
				);
			} else {
				$fo = null;
	...
	private function do_keep_options(array $args, array $state) {
		...
		if ($state['full'] && !isset($state['tables']['options'])) {
			...
			$keep_options = array_merge(
				slz_ext( 'backups' )->get_config( 'db.restore.keep_options' ),
				apply_filters('slz_ext_backups_db_restore_keep_options', array(), $state['full'])
			);
			...
			if ($state['step']) {
				...
				if (empty($option_name)) {
					return new WP_Error(
						...
						__('Failed to restore options keeping step', 'slz')
					);
				}
			} else {
				...
				while ( time() - $started_time < $timeout ) {
					if ($row = $wpdb->get_row($wpdb->prepare(
						...
						if (false === $wpdb->query(
							...
							return new WP_Error(
								'option_keep_fail',
								sprintf(__('Failed to keep option: %s', 'slz'), $state['step'])
								.($wpdb->last_error ? '. '. $wpdb->last_error : '')
							);
						}
					}
		...
		if (!empty($rename_sql)) {
			...
			}
			{
				...
				if ($rename_sql === $wpdb->last_query && $wpdb->last_error) {
					return new WP_Error(
						'tables_rename_fail',
						__('Tables rename failed.', 'slz') .' '. $wpdb->last_error
					);
				}
			}
		}
