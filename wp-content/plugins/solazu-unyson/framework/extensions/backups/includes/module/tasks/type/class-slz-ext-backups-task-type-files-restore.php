...
class SLZ_Ext_Backups_Task_Type_Files_Restore extends SLZ_Ext_Backups_Task_Type {
	public function get_type() {
		return 'files-restore';
	}

	public function get_title(array $args = array(), array $state = array()) {
		return __('Files Restore', 'slz');
	}
	...
	public function execute(array $args, array $state = array()) {
		...
		$upload_dir = slz_fix_path($upload_dir['basedir']);

		{
			if (empty($args['source_dir'])) {
				...
				$args['source_dir'] = slz_fix_path($args['source_dir']);

				if (empty($args['source_dir']) || !file_exists($args['source_dir'])) {
					return new WP_Error(
						'invalid_source_dir', __('Invalid source dir', 'slz')
					);
				}
			}
			...
			if (empty($args['destinations'])) {
				return new WP_Error(
					'no_source', __('Source dirs not specified', 'slz')
				);
			} else {
				$args['destinations'] = array_map('slz_fix_path', $args['destinations']);
			}

			{
				...
				$args['skip_dirs'] = array_merge($args['skip_dirs'], array(
					...
					slz_get_framework_directory() => true, // prevent framework delete, it must exist and continue task execution
				));
			}
		}
		...
		if (empty($state)) {
			// prepare destinations
			{
				...
				foreach ($args['destinations'] as $dir_id => $dir_path) {
					$dir_path = slz_fix_path($dir_path);
		...
		if ($state['fs_required']) {
			if (!SLZ_WP_Filesystem::has_direct_access(ABSPATH)) {
				if (empty($args['filesystem_args'])) {
					return new WP_Error(
						'fs_no_access', __('No filesystem access, credentials required', 'slz')
					);
				} elseif (!WP_Filesystem($args['filesystem_args'])) {
					return new WP_Error(
						'invalid_fs_credentials', __('No filesystem access, invalid credentials', 'slz')
					);
				}
			} else {
				if (!WP_Filesystem()) {
					return new WP_Error(
						'fs_init_fail', __('Filesystem init failed', 'slz')
					);
				}
			}
	...
	private function clear_dir($dir, $fs, $skip_dirs) {
		$included_hidden_names = slz_ext('backups')->get_config('included_hidden_names');

		if ($fs) {
			global $wp_filesystem; /** @var WP_Filesystem_Base $wp_filesystem */

			$fs_dir = slz_fix_path(SLZ_WP_Filesystem::real_path_to_filesystem_path($dir));

			if (empty($fs_dir)) {
				return new WP_Error(
					'dir_to_fs_failed',
					sprintf(__('Cannot convert Filesystem path: %s', 'slz'), $dir)
				);
			} elseif (false === ($list = $wp_filesystem->dirlist($fs_dir, true))) {
				return new WP_Error(
					'dir_list_failed',
					sprintf(__('Failed to list dir: %s', 'slz'), $dir)
				);
			}
			...
			foreach ($list as $file) {
				...
				if ($file['type'] === 'd') {
					...
					if (!$wp_filesystem->rmdir($fs_file_path, true)) {
						return new WP_Error(
							'dir_rm_fail',
							sprintf(__('Failed to remove dir: %s', 'slz'), $file_path)
						);
					}
				} else {
					if (!$wp_filesystem->delete($fs_file_path)) {
						return new WP_Error(
							'file_rm_fail',
							sprintf(__('Failed to remove file: %s', 'slz'), $file_path)
						);
					}
				}
			}
			...
			foreach ($files as $file_name) {
				...
				if (is_dir($file_path)) {
					...
					if (!slz_ext_backups_rmdir_recursive($file_path)) {
						return new WP_Error(
							'dir_rm_fail',
							sprintf(__('Failed to remove dir: %s', 'slz'), $file_path)
						);
	...
	private function copy_dir($source_dir, $destination_dir, $fs, $skip_dirs) {
		...
		if ($fs) {
			...
			$fs_source_dir = slz_fix_path(SLZ_WP_Filesystem::real_path_to_filesystem_path($source_dir));

			if (empty($fs_source_dir)) {
				return new WP_Error(
					'dir_to_fs_failed',
					sprintf(__('Cannot convert Filesystem path: %s', 'slz'), $source_dir)
				);
			} elseif (false === ($list = $wp_filesystem->dirlist($fs_source_dir, true))) {
				return new WP_Error(
					'dir_list_failed',
					sprintf(__('Failed to list dir: %s', 'slz'), $source_dir)
				);
			}
			...
			foreach ($list as $file) {
				...
				$fs_destination_file_path = slz_fix_path(SLZ_WP_Filesystem::real_path_to_filesystem_path(
				...
				if (empty($fs_destination_file_path)) {
					return new WP_Error(
						'path_to_fs_failed',
						sprintf(__('Cannot convert Filesystem path: %s', 'slz'), $destination_file_path)
					);
				}
				...
				if ($file['type'] === 'd') {
					...
					if (!$wp_filesystem->mkdir($fs_destination_file_path)) {
						return new WP_Error(
							'fs_mkdir_fail',
							sprintf(__('Failed to create dir: %s', 'slz'), $destination_file_path)
						);
					}
					...
					if (!$wp_filesystem->copy($fs_file_path, $fs_destination_file_path)) {
						return new WP_Error(
							'file_copy_fail',
							sprintf(__('Failed to copy file: %s', 'slz'), $file_path)
						);
					}
				}
			}
			...
			foreach ($names as $file_name) {
				...
				if (is_dir($file_path)) {
					...
					if (is_dir($destination_file_path)) {
						/**
						 * Some times empty directories are not deleted ( @see slz_ext_backups_rmdir_recursive() )
