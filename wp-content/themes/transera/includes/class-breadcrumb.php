...
class Transera_Breadcrumb {
	...
	public function get_breadcrumb() {
		return apply_filters( 'transera_get_breadcrumb', $this->crumbs, $this );
	}
	...
	private function add_crumbs_404() {
		$this->add_crumb( esc_html__( 'Error 404', 'transera' ) );
	}
	...
	private function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'post' != get_post_type( $post ) ) {
			...
			$cat = '';
			$post_options = get_post_meta( $post->ID, 'transera_post_options', true);
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( esc_html__( 'Posts tagged', 'transera' ).sprintf( ' &ldquo;%s&rdquo;', single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( esc_html__( 'Author', 'transera' ).': '.$userdata->display_name );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( esc_html__( 'Search results for', 'transera' ).sprintf( ' &ldquo;%s&rdquo;' , get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( esc_html__( 'Page', 'transera' ).' '.get_query_var( 'paged' ) );
		}
	}
}